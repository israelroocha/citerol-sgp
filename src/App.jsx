        ...
        <div className="sgp-scroll" style={{flex:1,overflow:"auto",minHeight:0}}>
          ...
          {tab==="info"&&<div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:10}}>
              {[["Cliente",order.client],["CNPJ",order.cnpj],["Razão Social",order.razaoSocial],["Vendedor",order.vendedor],["Telefone",order.tel],["E-mail",order.email],["Valor",fmtR(order.valor)],["Condição de Pgto",order.condicaoPagamento],["Centro de Custo",order.centroCusto||"—"],["Transportadora",order.transportadora||"—"],["Frete",fmtR(order.valorFrete||0)+(order.pagadorFrete?` · ${order.pagadorFrete}`:"")],["Prazo Faturamento do Pedido", _loadingDet?"⏳ Carregando...":(dataVencimento(order)?fmtD(dataVencimento(order)):"A definir (sem amostra aprovada)")],["Emissão",fmtD(order.dataFechamento||order.entradaAt)]].map(([k,v])=>(
                <div key={k} style={{background:C.gray50,borderRadius:6,padding:"10px 12px",border:`1px solid ${C.gray200}`}}>
                  <div style={{...F.body,fontSize:10,color:C.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:4}}>{k}</div>
...
// Cor por tipo pra identificação rápida no card e no cabeçalho do pedido.
function TagCentroCusto({cc,size="md"}){
  const v=String(cc||"").trim();
  if(!v) return null;
  const s=v.toLowerCase();
  let cor,bg,bd,label=v.toUpperCase();
  if(/concession/.test(s))      { cor="#4b2673"; bg="#efe8f7"; bd="#7c4bb8"; label="CONCESSIONÁRIA"; }
  else if(/licita|pregao|pregão|governo|publico|público/.test(s)) { cor="#7a2a06"; bg="#fce8d5"; bd="#c46a2f"; label="LICITAÇÃO"; }
  else if(/b2b|revend|distribu/.test(s)) { cor="#0e4f6e"; bg="#e2f1f8"; bd="#3a86ad"; label="B2B"; }
  else if(/corporat|empresa/.test(s)) { cor="#0d4d24"; bg="#e8f5ec"; bd="#4a8f5f"; label="CORPORATIVO"; }
  else if(/varejo|loja|b2c|consumidor/.test(s)) { cor="#8a4b00"; bg="#fdf0dd"; bd="#c98b2e"; label="VAREJO"; }
    ...
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      <div className="produto-row" style={{background:"#f0f0f0",border:"1px solid #c0c0c0",borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        <div style={{minWidth:0,flex:1}}>
          ...
          {(o.cnpj||o.endereco||o.clifor||o.inscricaoEstadual)&&<div style={{...F.body,fontSize:11,color:"#222",marginTop:2,fontWeight:600}}>{o.cnpj&&`CNPJ: ${o.cnpj}`}{o.inscricaoEstadual?((o.cnpj?" · ":"")+`IE: ${o.inscricaoEstadual}`):""}{o.clifor?((o.cnpj||o.inscricaoEstadual?" · ":"")+`Clifor: ${o.clifor}`):""}{o.endereco?((o.cnpj||o.clifor||o.inscricaoEstadual)?" · ":"")+o.endereco:""}</div>}
