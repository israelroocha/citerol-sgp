...
function ExecPorBordado({order,etapa,onAction,comDificuldade,setActionMsg,setActionDone,loadingDet,me,setTemPendencias}){
  ...
  const [executados, setExecutados] = useState(order.programacaoExecutados || []);
  const [assumindoKey, setAssumindoKey] = useState(null);
  const findAssumido = (fileName) => assumidos.find(a => a.fileName === fileName);
  const findExecutado = (fileName) => executados.find(a => a.nomeArquivo === fileName);
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {(() => {
        ...
        return (
          <div style={{background:"#fef3c7",border:`1.5px solid ${C.amber}`,borderLeft:`5px solid ${C.amber}`,borderRadius:8,padding:"12px 16px",display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{...F.body,fontSize:13,color:"#78350f",marginTop:2,fontWeight:600}}>
                Você tem <strong>{totalPendente} arquivo{totalPendente!==1?"s":""}</strong> anexado{totalPendente!==1?"s":""} localmente. Clique em <strong>"{BTN}"</strong> abaixo pra salvar no HubSpot. Se fechar sem confirmar, os anexos serão perdidos.
              </div>
            </div>
          </div>
        );
      ...
      {bordados.map((b,i)=>{
        ...
        return(
          <div key={k} style={{
            ...
            {jaExecutado&&<div style={{background:C.green+"14",border:`1.5px solid ${C.green}55`,borderRadius:8,padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
              <div style={{...F.body,fontSize:12,fontWeight:700,color:"#065f46",display:"flex",alignItems:"center",gap:6}}>
            ...
            {!bloqueado && !precisaAssumir && comDificuldade&&<div>
