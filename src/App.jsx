...
function ExecPorBordado({order,etapa,onAction,comDificuldade,setActionMsg,setActionDone,loadingDet,me,setTemPendencias}){
  ...
  // Sincroniza estado quando order enriquecer (o snapshot leve vem primeiro
  // sem esses campos; o enriched via /pedido-completo popula depois).
  useEffect(() => {
    if (order.programacaoExecutados) setExecutados(order.programacaoExecutados);
  }, [order.programacaoExecutados]);
  useEffect(() => {
    if (order.programacaoAssumidos) setAssumidos(order.programacaoAssumidos);
  }, [order.programacaoAssumidos]);
  // Normaliza nome pra comparação (remove ~PROG / ~AMOSTRA e whitespace)
  // Dados antigos foram gravados sem ~PROG; bordados_json tem ~PROG.
  // Sempre limpa nos dois lados antes de comparar.
  const nomeCompar = (n="") => (n||"").replace(/\s*~(PROG|AMOSTRA)/gi,"").trim();
  const findAssumido = (fileName) => {
    const alvo = nomeCompar(fileName);
    return assumidos.find(a => nomeCompar(a.fileName || a.nomeArquivo) === alvo);
  };
  const findExecutado = (fileName) => {
    const alvo = nomeCompar(fileName);
    return executados.find(a => nomeCompar(a.nomeArquivo || a.fileName) === alvo);
  };
