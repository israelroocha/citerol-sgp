...
// ─── PROGRAMAÇÃO: execução por bordado (imagem + dificuldade + arquivos) ──────
function ProgramacaoExec({order,onAction,setActionMsg,setActionDone}){
  const todos=(order.bordadosJson||[]).filter(b=>b&&(b.fileName||b.fileId));
  let bordados=todos.filter(b=>/~prog/i.test(b.fileName||""));
  if(!bordados.length) bordados=todos;                                   // legado: sem termo → todos
  if(!bordados.length) bordados=[{fileName:"Programação geral",fileId:"",fileUrl:""}]; // sem json
  ...
  const confirmar=async()=>{
    for(const [i,b] of bordados.entries()){
      ...
      if(!st.dificuldade){alert(`Defina a dificuldade do bordado: ${nomeLimpo(b.fileName)}`);return;}
      if(!st.files.length){alert(`Anexe ao menos um arquivo programado para: ${nomeLimpo(b.fileName)}`);return;}
    }
    setEnviando(true);
    try{
      ...
      for(const [i,b] of bordados.entries()){
        ...
        execucoes.push({nomeArquivo:nomeLimpo(b.fileName),dificuldade:st.dificuldade,arquivos});
      }
      const m=await onAction(order.id,"programacao_exec",{execucoes});
      setActionMsg(m||"Programação registrada.");setActionDone(true);
    }catch(e){alert("Erro ao enviar programação: "+e.message);}
    finally{setEnviando(false);}
  };
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      <div style={{background:C.gray50,border:`1px solid ${C.gray200}`,borderRadius:8,padding:"12px 16px"}}>
        <div style={{...F.title,fontSize:11,fontWeight:700,color:C.gray500,letterSpacing:"0.1em",marginBottom:4}}>PROGRAMAÇÃO DE BORDADO</div>
        <div style={{...F.body,fontSize:13,color:C.gray600}}>Para cada bordado: baixe a referência, informe a dificuldade e anexe o(s) arquivo(s) programado(s).</div>
      </div>
      {bordados.map((b,i)=>{
        ...
        return(
          <div key={k} style={{border:`1px solid ${C.gray200}`,borderRadius:10,padding:14,display:"flex",flexDirection:"column",gap:12}}>
            ...
            <div>
              <label style={{...F.body,fontSize:11,fontWeight:700,color:C.gray600,textTransform:"uppercase",letterSpacing:"0.06em",display:"block",marginBottom:6}}>Arquivo(s) programado(s)</label>
              <input id={`prog-input-${k}`} type="file" multiple accept=".emb,.dst,.pes,.jef,image/*" style={{display:"none"}}
      ...
      <button onClick={confirmar} disabled={enviando}
        ...
        <Ic n="send" s={15} c={C.white}/> {enviando?"Enviando...":"Confirmar programação"}
      </button>
    </div>
  );
}
...
function AcaoTab({order,me,uploadFile,setUploadFile,uploadName,setUploadName,obsText,setObsText,actionDone,setActionDone,actionMsg,setActionMsg,itemSel,itemDest,nSel,allDestDefined,skus,toggleItemSel,selAllItems,setDestSel,setDestAll,setDestOne,onAction,isMobile}){
  ...
  if(etapa==="Programação"){
    return <ProgramacaoExec order={order} onAction={onAction} setActionMsg={setActionMsg} setActionDone={setActionDone}/>;
  }
...
function AppInner(){
  ...
  const handleAction=async(orderId,tipo,payload)=>{
    ...
    try{
      ...
      // ── PROGRAMAÇÃO (execução por bordado: dificuldade + arquivos) ─────────────
      else if(tipo==="programacao_exec"){
        if(!bordadoId){ alert("Pedido sem negócio de Bordado associado."); return; }
        if(!payload.execucoes||!payload.execucoes.length){ alert("Nenhum bordado para programar."); return; }
        const next="Amostra Digital";
        const res=await apiFetch(`/programacao-exec/${bordadoId}`,"POST",{
          execucoes:payload.execucoes,
          propriedade:ETAPA_PROPRIEDADE["Programação"],
          novaEtapa:ETAPA_STAGE_ID[next],
          nota:`Programação → ${next}`,
          ctx,
        });
        if(res.error) throw new Error(res.error);
        resultMsg="Programação registrada e enviada para Amostra Digital.";
      }
