...
// ─── VERSÃO ───────────────────────────────────────────────────────────────────
const SGP_VERSION = "v2.9.81-debug";
...
function ExecPorBordado({order,etapa,onAction,comDificuldade,setActionMsg,setActionDone,loadingDet,me,setTemPendencias}){
  ...
  const _debugExec = order.programacaoExecutados;
  const _debugInfo = {
    tipo: typeof _debugExec,
    isArray: Array.isArray(_debugExec),
    length: _debugExec?.length,
    hasNomeArquivo: _debugExec?.[0]?.nomeArquivo,
    hasFileName: _debugExec?.[0]?.fileName,
  };
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {/* DEBUG VISUAL — remove depois */}
      {etapa === "Programação" && <div style={{background:"#fef3c7",border:"2px solid #f59e0b",borderRadius:8,padding:"12px 16px",fontFamily:"monospace",fontSize:11,color:"#78350f"}}>
        <div style={{fontWeight:700,marginBottom:6}}>🐛 DEBUG (temporário)</div>
        <div>tipo: <strong>{_debugInfo.tipo}</strong> · isArray: <strong>{String(_debugInfo.isArray)}</strong> · length: <strong>{String(_debugInfo.length)}</strong></div>
        <div>tem nomeArquivo? <strong>{String(!!_debugInfo.hasNomeArquivo)}</strong> · tem fileName? <strong>{String(!!_debugInfo.hasFileName)}</strong></div>
        <div style={{marginTop:6,padding:6,background:"#fff",borderRadius:4,fontSize:10,wordBreak:"break-all"}}>
          {_debugExec ? JSON.stringify(_debugExec).slice(0, 500) : "(vazio/null)"}
        </div>
      </div>}
