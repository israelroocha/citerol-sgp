  ...
  return (
    <div style={{padding:0}}>
      ...
      <div style={{padding:"12px 32px",background:C.gray50,borderBottom:`1px solid ${C.gray200}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        ...
        <select value={filtroVendedor} onChange={e=>setFiltroVendedor(e.target.value)} style={{padding:"8px 12px",fontSize:12,border:`1.5px solid ${C.gray200}`,background:C.white,borderRadius:6,color:C.gray700,cursor:"pointer",...F.body}}>
          ...
          <div style={{width:320,background:C.white,borderRight:`1px solid ${C.gray200}`,overflowY:"auto"}}>
            <div style={{padding:"14px 20px",background:"#f0f9ff",borderLeft:"3px solid #0369a1",borderBottom:`1px solid ${C.gray100}`}}>
              ...
              <div style={{...F.body,fontSize:11,color:"#0369a1",fontWeight:700,marginTop:6}}>
                📁 {pedidos.length} pedido{pedidos.length!==1?"s":""} · {pedidos.reduce((s,p)=>s+p.totalArquivos,0)} arquivos
              </div>
            </div>
            ...
            {!loading && pedidos.length===0 && <div style={{padding:20,textAlign:"center",color:C.gray400,...F.body,fontSize:12}}>Nenhum pedido com arquivos encontrado.</div>}
            {pedidos.map(p => (
