...
function Direcionamento({orders,setOrders,onOpen,slaCfg,user}){
  ...
  return(
    <div style={{padding:24,display:"flex",flexDirection:"column",gap:20}}>
      ...
      {prontos.length===0&&<div style={{...F.body,color:C.gray400,textAlign:"center",padding:48,fontSize:13,background:C.white,borderRadius:8,border:`1px solid ${C.gray200}`}}>{aba==="saira"?"Nenhum pedido saiu do aguardando amostra no momento.":"Nenhum pedido aguardando direcionamento."}</div>}
      {prontos.map(o=>{
...
function Fila({title,etapa,orders,onOpen,actionLabel,actionColor=C.green,slaCfg,endpoint,finalizado,filtroBordador,setFiltroBordador,topoExtra,subTabsReprog,subTabExecutados,user}){
  ...
  // Ordena por URGÊNCIA DE SLA: o mais "estourado" primeiro (casa com o badge
  // ATRASADO/EM RISCO/NO PRAZO e a barra Xh/Yh do card). htd = horas úteis
  // restantes até o prazo da etapa (negativo = já venceu; quanto menor, mais
  // atrasado). Cobre a Programação, onde não há dataVencimento formal mas o SLA
  // (etapaAt + horas úteis configuradas) já define o vencimento efetivo.
  mine=[...mine].sort((a,b)=>{
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:5}}>
                    ...
                    {etapa==="Amostra Física"&&o.colocadoParaBordar&&<span style={{display:"inline-flex",alignItems:"center",gap:4,background:"#7c3aed",color:C.white,borderRadius:6,padding:"2px 9px",...F.body,fontSize:10,fontWeight:700,letterSpacing:"0.04em"}}>🧵 NA MÁQUINA</span>}
                    {/* Badge de status */}
                    {finalizado
                      ?(()=>{
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
                  {etapa==="Amostra Física"&&<button
                    onClick={async(e)=>{e.stopPropagation();try{await apiFetch(`/colocar-para-bordar/${o.vendasId}`,"POST",{remover:o.colocadoParaBordar===true,ctx:{executor:user?.nome||user?.email||"Usuário SGP"}});carregar();}catch(err){alert("Erro: "+err.message);}}}
                    title={o.colocadoParaBordar?"Clique para desmarcar":"Marcar que já foi colocado na máquina"}
                    style={{display:"inline-flex",alignItems:"center",gap:6,border:o.colocadoParaBordar?`1.5px solid ${C.green}`:`1.5px solid #7c3aed`,background:o.colocadoParaBordar?C.green+"12":"#7c3aed",color:o.colocadoParaBordar?"#065f46":C.white,borderRadius:7,padding:"7px 12px",...F.body,fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
                    {o.colocadoParaBordar?<><Ic n="check" s={13} c="#065f46"/> Na máquina</>:<>🧵 Colocar para Bordar</>}
                  </button>}
