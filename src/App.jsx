...
const PERMISSOES_ESPECIAIS = [
  {id:"voltar_etapa", label:"Alterar Etapa de Pedido", icon:"arrow", desc:"Permite mover um pedido para outra etapa — anterior (voltar) ou posterior (avançar)."},
];
      ...
      <div style={{background:C.white,borderRadius:isMobile?0:10,width:"100%",maxWidth:900,maxHeight:isMobile?"100dvh":"92vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.gray200}`,display:"flex",flexDirection:"column",gap:12,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            ...
            {me&&me.admin&&order.temBordado===false&&(order.bordadosJson||[]).some(b=>b&&b.type==="bordado"&&(b.sku||b.productId))&&<button
              disabled={gerandoBordado}
              onClick={async()=>{
                if(!confirm("Este pedido está marcado como SEM bordado, mas tem arquivos de bordado anexados. Gerar o bordado na Programação agora?"))return;
                setGerandoBordado(true);
                try{
                  const r=await apiFetch(`/gerar-bordado/${order.vendasId}`,"POST",{ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
                  if(r.success){ alert(`Bordado gerado com ${r.programacoes} programação(ões). O pedido vai aparecer na Programação em instantes.`); onClose(); }
                  else alert("Erro: "+(r.error||"desconhecido"));
                }catch(e){alert("Erro: "+e.message);}
