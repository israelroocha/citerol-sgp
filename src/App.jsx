...
// Depois de uma ação que muda a situação do pedido (cancelar bordado, corrigir
// pra silk/DTF, cancelar pedido...), o quadro precisa ser refeito. Sem isto o
// modal fechava e o card continuava exatamente onde estava — a ação tinha
// funcionado no HubSpot, mas na tela parecia que o clique não pegou.
// Mesma receita do handleAction: tira o card na hora e reconstrói atrás.
function recarregarAposAcao(order){
  try{ if(order?.vendasId) snapRemoverPedido(order.vendasId, order._etapaOrigem||order.etapa); }catch{}
  setTimeout(()=>{ try{ triggerRefresh(); }catch{} _fetchSnap(false); }, 1500);
}
...
const ICONS = {
  ...
  // Faltava no mapa: sem a entrada o menu renderiza o item sem ícone nenhum.
...
const NAV_ITEMS = [
  ...
  // Visão tabular da carteira, ordenável e exportável — relatório, não fila.
  {id:"relatorio_pedidos",       label:"Relatório de Pedidos",     icon:"chart",   grupo:"Indicadores"},
  ...
  // Auditoria dos pedidos que o quadro esconde por terem perdido o negócio de
  // Vendas. Sumir da operação não pode virar sumir do controle.
  {id:"pedidos_sem_venda", label:"Pedidos sem Venda", icon:"alert", grupo:"Sistema"},
];
...
const normalizarCard=(o,etapa)=>({
  ...
  // [FRETE] previsto (cotação) × realizado (expedição) — ver VolumesComparativo
  freteVolumes:o.freteVolumes||null,
  ...
  temBordado:o.temBordado!==false,temSilkDtf:o.temSilkDtf===true,silkDtfConcluido:o.silkDtfConcluido===true,
  ...
  // Contexto de quando o pedido está pausado (aguardando / pendência).
  motivo:o.motivo||"",pedidoDependencia:o.pedidoDependencia||"",
  dependenciaLinx:o.dependenciaLinx||"",etapaAnterior:o.etapaAnterior||"",
  entrouEsperaEm:o.entrouEsperaEm||null,
...
function ExecPorBordado({order,etapa,onAction,comDificuldade,setActionMsg,setActionDone,loadingDet,me,setTemPendencias}){
  ...
  // [OBS DO BORDADO] Um arquivo vale para vários produtos, e CADA registro do
  // bordados_json pode trazer observação própria. A tela mostrava só produto e
  // posição — o que a programação precisa ler (o que fazer com a arte) ficava
  // invisível, embora aparecesse na folha impressa e no HubSpot.
  // Junta as observações de todos os registros do mesmo arquivo, sem repetir.
  const obsDoArquivo=(b)=>{
    const fid=b&&b.fileId?String(b.fileId):("nome:"+((b&&b.fileName)||""));
    const vistos=new Set(); const out=[];
    for(const r of ((order.bordadosJson||[]).filter(x=>x&&(x.fileId?String(x.fileId):("nome:"+(x.fileName||"")))===fid))){
      for(const [rot,txt] of [["PROGRAMAÇÃO",r.obs_programacao],["BORDADO",r.obs_bordado]]){
        const t=String(txt||"").trim();
        if(!t)continue;
        const k=rot+"|"+t.toLowerCase();
        if(vistos.has(k))continue;
        vistos.add(k); out.push({rot,txt:t});
      }
    }
    return out;
  };
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {/* [INSTRUÇÕES DO PEDIDO] Observação do vendedor e informação importante.
          Estavam na folha impressa e no HubSpot, mas não nesta tela — quem
          programa o bordado precisa dessas instruções ANTES de executar, e
          estava tendo que abrir o pedido em outro lugar pra ler. */}
      {(() => {
        const info = String(order.infoImportante||"").trim();
        const obsProd = String(order.obsProdutos||"").split(/\s*\|\s*/).map(x=>x.trim()).filter(Boolean);
        // Sem repetir: a mesma frase costuma vir na obs de vários produtos.
        const obsUnicas = [...new Set(obsProd.map(x=>x))];
        if(!info && !obsUnicas.length) return null;
        return (
          <div style={{background:"#fff8e6",border:"1.5px solid #f0c36d",borderLeft:"5px solid #d99b1c",borderRadius:8,padding:"12px 16px"}}>
            <div style={{...F.title,fontSize:11,fontWeight:800,color:"#7a4b00",letterSpacing:"0.07em",marginBottom:8}}>
              INSTRUÇÕES DO PEDIDO
            </div>
            {info&&(
              <div style={{marginBottom:obsUnicas.length?10:0}}>
                <div style={{...F.body,fontSize:10,fontWeight:800,color:"#a35c00",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:2}}>
                  Informação importante
                </div>
                <div style={{...F.body,fontSize:13,color:"#5c3a00",lineHeight:1.5,whiteSpace:"pre-wrap",wordBreak:"break-word",fontWeight:600}}>{info}</div>
              </div>
            )}
            {obsUnicas.length>0&&(
              <div>
                ...
                {obsUnicas.map((o,i)=>(
                  <div key={i} style={{...F.body,fontSize:13,color:"#5c3a00",lineHeight:1.5,padding:"2px 0",whiteSpace:"pre-wrap",wordBreak:"break-word"}}>• {o}</div>
                ))}
              </div>
            )}
          </div>
              ...
              return (
                <div key={k} style={{display:"grid",gridTemplateColumns:gcols,gap:10,alignItems:"center",padding:"10px 14px",borderLeft:`1px solid ${C.gray200}`,borderRight:`1px solid ${C.gray200}`,borderBottom:`1px solid ${C.gray100}`,background:isSel?"#fdf3f3":disp?"#fffbeb":jaExecutado?"#f0fdf4":bloqueado?C.gray50:"#fff",opacity:bloqueado?0.65:1}}>
                  ...
                  <div style={{minWidth:0}}>
                    ...
                    {/* Observações do bordado: é a instrução do que fazer com
                        a arte. Fica em destaque âmbar, como na folha impressa. */}
                    {obsDoArquivo(b).map((o,oi)=>(
                      <div key={oi} style={{...F.body,fontSize:11.5,color:"#7a4b00",background:"#fff8e6",
                        border:"1px solid #f0c36d",borderLeft:"3px solid #d99b1c",borderRadius:4,
                        padding:"5px 8px",marginTop:5,lineHeight:1.45,whiteSpace:"pre-wrap",wordBreak:"break-word"}}>
                        <strong style={{fontSize:10,letterSpacing:"0.04em"}}>[{o.rot}]</strong> {o.txt}
                      </div>
                    ))}
...
function AcaoTab({order,me,uploadFiles,setUploadFiles,obsText,setObsText,actionDone,setActionDone,actionMsg,setActionMsg,itemSel,itemDest,nSel,allDestDefined,skus,itensDirecionaveis,toggleItemSel,selAllItems,setDestSel,setDestAll,setDestOne,onAction,isMobile,loadingDet,setTemPendencias}){
  ...
  // ── ETAPAS DE PAUSA (Aguardando Outro Pedido / Pendência Comercial) ───────
  // O pedido está parado esperando algo de fora. Quem encontra ele pela busca
  // do Dashboard precisa conseguir liberar aqui mesmo — antes a ação só existia
  // na caixa específica, e quem abria pelo Dashboard ficava sem saída.
  if(etapa==="Aguardando Outro Pedido"||etapa==="Pendência Comercial"){
    const ehAguardando = etapa==="Aguardando Outro Pedido";
    const rota = ehAguardando ? "/aguardando-outro-pedido/sair" : "/pendencia-comercial/sair";
    const dealId = order.posvendaId || order.bordadoId;
    return(
      <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:"#1d4ed8"+"0e",border:"1px solid #1d4ed833",borderRadius:8,padding:"12px 16px"}}>
          <div style={{...F.title,fontSize:11,fontWeight:700,color:"#1d4ed8",letterSpacing:"0.1em",marginBottom:6}}>
            {ehAguardando?"AGUARDANDO OUTRO PEDIDO":"PENDÊNCIA COMERCIAL"}
          </div>
          <div style={{...F.body,fontSize:13,color:C.gray700,lineHeight:1.5}}>
            {ehAguardando
              ? <>Este pedido está esperando outro para seguir. Ao liberar, ele volta para a etapa onde estava{order.etapaAnterior?<> (<strong>{order.etapaAnterior}</strong>)</>:null}.</>
              : <>Este pedido está parado por pendência comercial. Ao liberar, ele volta para a etapa onde estava{order.etapaAnterior?<> (<strong>{order.etapaAnterior}</strong>)</>:null}.</>}
          </div>
          {(order.pedidoDependencia||order.dependenciaLinx)&&
            <div style={{...F.body,fontSize:12.5,color:"#1e40af",marginTop:8,fontWeight:600}}>
              Aguardando o pedido: {order.dependenciaLinx?`PED - ${order.dependenciaLinx}`:order.pedidoDependencia}
            </div>}
          {order.motivo&&<div style={{...F.body,fontSize:12.5,color:C.gray700,marginTop:6,whiteSpace:"pre-wrap"}}>
            <strong>Motivo da espera:</strong> {order.motivo}
          </div>}
        </div>
        <div>
          ...
          <textarea value={obsText} onChange={e=>setObsText(e.target.value)} rows={3}
            placeholder={ehAguardando?"Ex: o outro pedido foi faturado; material chegou...":"Ex: o comercial resolveu com o cliente..."}
        ...
        <button onClick={async()=>{
            if(!obsText.trim()){alert("O motivo do retorno é obrigatório.");return;}
            if(!dealId){alert("Pedido sem negócio associado.");return;}
            setUploading(true);
            try{
              const r=await apiFetch(rota,"POST",{dealId,motivo:obsText.trim(),
                ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
              if(r.success){
                setActionMsg(`Pedido liberado.${typeof r.horasNessaEspera==="number"?` Ficou ${r.horasNessaEspera.toFixed(1)}h em espera.`:""}`);
                setActionDone(true);
              } else alert("Erro: "+(r.error||"desconhecido"));
            }catch(e){alert("Erro: "+e.message);}
          ...
          disabled={uploading}
          style={{background:uploading?"#ccc":C.green,color:C.white,border:"none",borderRadius:8,padding:"13px 24px",...F.body,fontWeight:700,fontSize:14,cursor:uploading?"wait":"pointer",display:"inline-flex",alignItems:"center",gap:8,alignSelf:"flex-start"}}>
          <Ic n="check" s={16} c={C.white}/> {uploading?"Liberando...":"Marcar como resolvido e voltar"}
        </button>
      </div>
    );
  }
  ...
  // ── PENDENTE PAGAMENTO ────────────────────────────────────────────────────
  // Cobrança: registrar tratativa e, se for o caso, liberar sem o aval do ERP.
  if(etapa==="Pendente Pagamento"){
    return <PendentePagamentoAcao order={order} me={me} setActionDone={setActionDone}/>;
  }
  ...
  if(etapa==="Conferência e Direcionamento" && order.temBordado===false){
    // [SILK/DTF] A peça com silk ou DTF ainda precisa da aplicação. Não é etapa
    // (é personalização), então o controle é uma FLAG: enquanto a produção não
    // marcar como pronta, o pedido não segue — senão ia pra Expedição com a
    // peça crua. A mesma trava existe no worker; isto aqui é só a interface.
    const silkPendente = order.temSilkDtf===true && order.silkDtfConcluido!==true;
    return(
      <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
        <div style={{background:C.gray50,border:`1px solid ${C.gray200}`,borderRadius:8,padding:"12px 16px"}}>
        ...
        {silkPendente&&(
          <div style={{background:"#f5f3ff",border:"1.5px solid #7c3aed55",borderLeft:"5px solid #7c3aed",borderRadius:8,padding:"12px 14px"}}>
            <div style={{...F.title,fontSize:11,fontWeight:800,color:"#6d28d9",letterSpacing:"0.06em",marginBottom:4}}>SILK / DTF PENDENTE</div>
            <div style={{...F.body,fontSize:12.5,color:C.gray700,lineHeight:1.5}}>
              Este pedido tem peça de <strong>silk/DTF</strong> que ainda não foi aplicada. A produção precisa
              concluir a aplicação antes de o pedido seguir para a Expedição.
            </div>
            <div style={{marginTop:10}}>
              <SilkDtfConcluirBtn order={order} me={me}/>
            </div>
          </div>
        )}
        {order.temSilkDtf===true&&order.silkDtfConcluido===true&&(
          <div style={{...F.body,fontSize:12.5,fontWeight:600,color:"#047857",background:"#ecfdf5",border:"1px solid #04785744",borderRadius:7,padding:"9px 12px",display:"flex",alignItems:"center",gap:7}}>
            <Ic n="check" s={14} c="#047857"/> Silk/DTF aplicado — pedido liberado para a Expedição.
          </div>
        )}
        <button disabled={silkPendente}
          onClick={()=>{if(silkPendente)return;onAction(order.id,"conferir_direcionamento",{});setActionDone(true);}}
          title={silkPendente?"Conclua o silk/DTF antes de enviar para a Expedição":""}
          style={{alignSelf:"flex-start",background:silkPendente?C.gray200:C.green,color:silkPendente?C.gray500:C.white,border:"none",borderRadius:7,padding:"11px 22px",cursor:silkPendente?"not-allowed":"pointer",...F.body,fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:7}}>
          <Ic n="check" s={15} c={silkPendente?C.gray500:C.white}/> Conferido — enviar para Expedição
        </button>
      </div>
    );
  }
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {/* [FRETE] Previsto × realizado. Fica ANTES da transportadora e do valor
          porque é a informação que decide a contratação: o valor cotado se
          apoia no previsto, e quem manda é o que a expedição de fato embalou. */}
      {etapa==="Análise de Frete"&&<VolumesComparativo fv={order.freteVolumes}/>}
      ...
      {/* [FRETE] Em quantas e quais caixas consolidar. Vem antes da bipagem
          porque é o que o conferente precisa saber ANTES de embalar. */}
      {etapa==="Expedição"&&order.vendasId&&(
        <VolumesExpedicao order={order} user={me} onSalvo={()=>onAction&&onAction("refresh")}/>
      )}
      {etapa==="Expedição"&&(
...
function OrderModal({order: _orderLeve,me,onClose,usuarios,onAction,isMobile,slaCfg}){
  ...
  const order=_enriched?{
    ...
    // ⚠ Ter negócio de Bordado NÃO significa ter bordado. Ele continua existindo
    // depois de encerrado (correção silk/DTF, cancelar bordado) — guardamos pra
    // auditoria. Usar `!!bordadoId` fazia o pedido corrigido voltar a pedir
    // direcionamento (caso 219085 / MED TECH).
    // Ordem: flag explícita do detalhe > bordado ATIVO > o que o quadro sabia.
    // Ordem: bordado ATIVO vence tudo (trabalho em curso). Sem bordado ativo,
    // vale o que o detalhe disser; o quadro entra só enquanto ele não chegou.
    temBordado: _enriched.bordadoAtivo===true ? true
      : (_enriched.temBordado===false ? false
        : (_enriched.temBordado===true || _orderLeve.temBordado===true)),
    bordadoAtivo: _enriched.bordadoAtivo===true,
    flagBordadoDivergente: _enriched.flagBordadoDivergente===true,
    ...
    // [FRETE] o enriched é lido na hora (traz o que a expedição acabou de
    // registrar); o snapshot entra quando o detalhe ainda não carregou.
    freteVolumes: _enriched.freteVolumes || _orderLeve.freteVolumes || null,
    ...
    // obsProdutos (observação do vendedor nos itens) só existe no snapshot —
    // o detalhe não traz. Sem preservar aqui, a instrução some ao abrir o card.
    obsProdutos: _orderLeve.obsProdutos || _enriched.obsProdutos || "",
  ...
  const ETAPAS_COM_ACAO=["Em Separação","Conferência e Direcionamento","Programação","Amostra Digital","Amostra Física","Aprovação de Amostra Digital","Aprovação de Amostra Física","Bordado Interno","Bordado Externo","Bordado Interno e Externo","Expedição","Análise de Frete",
    // Pendente Pagamento: cobrar e liberar são ações do pedido. Sem isto elas
    // só existiam nos botões da fila — quem abria o card pra conferir valor e
    // condição não tinha o que fazer e precisava voltar pra lista.
    "Pendente Pagamento",
    ...
    // Etapas de PAUSA: quem acha o pedido pela busca do Dashboard precisa
    // conseguir liberá-lo ali mesmo, sem ter que caçar a caixa certa no menu.
    "Pendência Comercial","Aguardando Outro Pedido"];
  ...
  // ⚠ O fallback "nenhum item marcado → direciona todos" existe pra pedido
  // legado, que nasceu sem a flag nos itens. Mas ele não pode valer quando os
  // itens foram DESMARCADOS de propósito (correção silk/DTF, cancelar bordado):
  // aí o certo é não haver nada pra direcionar.
  const itensDirecionaveis=itensComBordado.length ? itensComBordado
    : (order.temBordado===false ? [] : order.items);
        ...
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.gray200}`,display:"flex",flexDirection:"column",gap:12,flexShrink:0}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              ...
              {/* Pedido órfão: o negócio de Vendas foi excluído no HubSpot. Sem
                  ele faltam campos (nº Linx, frete, natureza, itens de venda).
                  O selo evita que a tela vazia pareça erro do sistema. */}
              {order.semVendas===true&&<Tag label="Sem negócio de Vendas" color={C.red}/>}
              {/* Bordado rodando com a flag "sem bordado" no HubSpot. O SGP
                  segue o bordado, mas o dado precisa ser corrigido lá. */}
              {order.flagBordadoDivergente===true&&<Tag label="Flag de bordado divergente" color={C.amber}/>}
...
function Bonificacoes({onOpen,user}){
  ...
  const concluir=async(o)=>{
    ...
    const aviso=o.temBordado!==false&&!o.amOk
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:220}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  ...
                  <TagCentroCusto cc={o.centroCusto}/>
                  <Tag label={o.temBordado!==false?"Com bordado":"Sem bordado"} color={o.temBordado!==false?C.purple:C.gray600}/>
...
function MiniAcaoAdmin({rota,rotulo,rotuloExec,vazio,linha}){
  ...
  const executar=async()=>{
    ...
    try{
      ...
      if(r.error){alert("Erro: "+r.error);return;}
      // Cada endpoint chama o resultado do que fez: movidos, corrigidos, feitos.
      // Sem contemplar os três, o aviso mostrava o total consultado em vez do
      // que realmente foi alterado.
      const n = r.movidos ?? r.corrigidos ?? r.feitos ?? r.total ?? 0;
      const errs = (r.erros||[]).length;
      alert(`Pronto: ${n} registro(s) processado(s).` + (errs?` ${errs} com erro — veja o console.`:""));
      if(errs) console.warn("[SGP] erros na ação admin:", r.erros);
      setPrevia(null);
    }catch(e){alert("Erro: "+e.message);}
    finally{setCarregando(false);}
  };
...
function SilkDtf({onOpen,slaCfg,user}){
  ...
  const carregar=()=>{
    ...
    apiFetch("/silk-dtf")
      .then(res=>{
        // Silk/DTF não é etapa — é personalização da peça. A lista é virtual:
        // vem de todas as etapas do quadro. Por isso preserva a etapa REAL de
        // cada pedido em vez de carimbar "Silk/DTF" em todo mundo.
        if(res.success)setPedidos(ordenarPorPrioridade((res.data||[]).map(o=>normalizarCard(o,o.etapa))));
        else setLoadError(res.error||"Erro desconhecido");
      })
      .catch(e=>setLoadError(e.message))
      .finally(()=>setLoading(false));
  ...
  const concluir=async(o)=>{
    if(!o.posvendaId){alert("Pedido sem negócio de Pós-venda.");return;}
    if(!confirm(`Confirmar que o silk/DTF do pedido ${o.pedidoLinx||o.vendasId} está aplicado?\n\nO pedido não muda de etapa — fica liberado para a conferência seguir para a Expedição.`))return;
    setAgindo(p=>({...p,[o.id]:true}));
    try{
      const r=await apiFetch("/silk-dtf-concluido/"+o.posvendaId,"POST",{ctx:{executor:user?.nome||"Usuário SGP"}});
      if(r.success){setFeito(p=>({...p,[o.id]:"aplicado"}));setTimeout(carregar,1200);}
      else alert("Erro: "+(r.error||"desconhecido"));
    }catch(e){alert("Erro: "+e.message);}
    finally{setAgindo(p=>({...p,[o.id]:false}));}
  };
  ...
  return(
    <div style={{padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}>
        <PageH title="Silk / DTF" sub="Peças de silk ou DTF aguardando aplicação. Enquanto não forem marcadas como aplicadas, o pedido não segue para a Expedição." onRefresh={carregar} refreshing={loading}/>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:220}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  ...
                  <ETag etapa={o.etapa}/>
                  <Tag label="Silk / DTF" color={C.purple}/>
                </div>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                ...
                {done
                  ? <span style={{background:C.green+"18",color:"#065f46",border:`1px solid ${C.green}55`,borderRadius:6,padding:"9px 16px",...F.body,fontWeight:700,fontSize:12,display:"inline-flex",alignItems:"center",gap:6}}>
                      <Ic n="check" s={14} c="#065f46"/> Marcado como aplicado
                    </span>
                  : o.silkDtfConcluido===true
                    ? <span style={{...F.body,fontSize:12,fontWeight:700,color:"#047857",display:"inline-flex",alignItems:"center",gap:5}}>
                        <Ic n="check" s={13} c="#047857"/> Aplicado
                      </span>
                    : <Btn label={busy?"Marcando...":"Silk/DTF aplicado"} icon="check" variant="success" disabled={!!busy} onClick={()=>concluir(o)}/>}
              </div>
            </div>
...
function Direcionamento({orders,setOrders,onOpen,slaCfg,user}){
  ...
  slaDesdeAprovacaoAmostra:o.slaDesdeAprovacaoAmostra===true,
            ...
            slaDesdeAprovacaoAmostra:o.slaDesdeAprovacaoAmostra===true,
            temBordado:o.temBordado!==false,temSilkDtf:o.temSilkDtf===true,silkDtfConcluido:o.silkDtfConcluido===true,dataVencimento:o.dataVencimento||null,
            items:(o.items||[]).map(it=>({
...
function Dashboard({onOpen,slaCfg}){
  ...
  const loading = snap.loading && !snap.data;
  // [BUSCA] O quadro só tem pedido EM ABERTO. Quem procura um pedido pelo
  // número não sabe (nem tem que saber) se ele já foi faturado — e a busca
  // vinha vazia, como se o pedido não existisse.
  // Os finalizados são carregados SOB DEMANDA: só quando há busca, e uma vez
  // por sessão. Carregar sempre custaria uma chamada pesada a cada abertura do
  // Dashboard, para um caso que é minoria.
  const [finalizados,setFinalizados]=useState(null);   // null = ainda não buscou
  const [carregandoFin,setCarregandoFin]=useState(false);
  // [BUSCA] As ocorrências vivem num funil PRÓPRIO (devolução, reclamação) e
  // não entram no quadro — têm telas separadas. Quem procura o número do pedido
  // não sabe disso e a busca voltava vazia, igual acontecia com os faturados.
  const [ocorrencias,setOcorrencias]=useState(null);
  const [carregandoOc,setCarregandoOc]=useState(false);
  ...
  const q=busca.trim().toLowerCase();
  // Carrega os finalizados na primeira busca com 3+ caracteres. Abaixo disso a
  // busca é ampla demais e traria a base inteira sem utilidade.
  useEffect(()=>{
    if(q.length<3||finalizados!==null||carregandoFin)return;
    setCarregandoFin(true);
    apiFetch("/finalizados")
      .then(r=>setFinalizados(r&&r.success?(r.data||[]).map(o=>normalizarCard(o,"Finalizado")):[]))
      .catch(()=>setFinalizados([]))
      .finally(()=>setCarregandoFin(false));
  },[q,finalizados,carregandoFin]);
  // Mesma regra para as ocorrências: só na primeira busca, uma vez por sessão.
  useEffect(()=>{
    if(q.length<3||ocorrencias!==null||carregandoOc)return;
    setCarregandoOc(true);
    apiFetch("/ocorrencias")
      .then(r=>setOcorrencias(r&&r.success?(r.data||[]).map(o=>normalizarCard(o,o.etapa||"Ocorrência")):[]))
      .catch(()=>setOcorrencias([]))
      .finally(()=>setCarregandoOc(false));
  },[q,ocorrencias,carregandoOc]);
  ...
  // Mesmo critério de busca do quadro — muda só a origem dos dados.
  // O critério de busca é o mesmo nos três blocos (aberto, faturado, ocorrência).
  // Uma função só evita que eles saiam do ar um do outro com o tempo.
  const casaBusca=(o,s)=>(o.client||"").toLowerCase().includes(s)
    ||String(o.id||"").toLowerCase().includes(s)
    ||String(o.pedidoLinx||"").toLowerCase().includes(s)
    ||String(o.vendasId||"").includes(s)
    ||(o.cnpj||"").toLowerCase().includes(s);
  const passaFiltros=(o)=>{
    if(centro&&o.centroCusto!==centro)return false;
    if(bordadoF==="com"&&o.temBordado===false)return false;
    if(bordadoF==="sem"&&o.temBordado!==false)return false;
    return true;
  };
  const ocorrenciasAchadas=(!q||q.length<3)?[]:(ocorrencias||[]).filter(o=>passaFiltros(o)&&casaBusca(o,q));
  const finalizadosAchados=(!q||q.length<3)?[]:(finalizados||[]).filter(o=>{
    if(centro&&o.centroCusto!==centro)return false;
    if(bordadoF==="com"&&o.temBordado===false)return false;
    if(bordadoF==="sem"&&o.temBordado!==false)return false;
  ...
  return(
    <div style={{padding:24,display:"flex",flexDirection:"column",gap:18}}>
      ...
      {grupo==="aberto"&&!loading&&<>
        ...
        {/* [BUSCA] Ocorrência corre num funil separado e não está no quadro.
            Bloco próprio, com a tarja de ocorrência, pra ninguém confundir com
            pedido normal em produção. */}
        {q.length>=3&&(carregandoOc||ocorrenciasAchadas.length>0)&&(
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <span style={{...F.title,fontSize:12,fontWeight:800,color:C.red,letterSpacing:"0.06em"}}>
                OCORRÊNCIAS
              </span>
              {carregandoOc
                ? <span style={{...F.body,fontSize:12,color:C.gray400}}>procurando...</span>
                : <span style={{...F.body,fontSize:11,fontWeight:700,color:C.white,background:C.red,borderRadius:10,padding:"1px 8px"}}>
                    {ocorrenciasAchadas.length}
                  </span>}
              <span style={{...F.body,fontSize:12,color:C.gray400}}>— devolução, reclamação e ajuste</span>
            </div>
            {ocorrenciasAchadas.length>0&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>
                {ocorrenciasAchadas.map(o=>(
                  <OCard key={"oc-"+(o.id||"")+(o.etapa||"")} order={o} onClick={()=>onOpen(o)} slaCfg={slaCfg}/>
                ))}
              </div>
            )}
          </div>
        )}
        {/* [BUSCA] Pedido já faturado não está no quadro. Quando a busca acha
            um, ele sai num bloco à parte — separado de propósito, pra ninguém
            confundir carteira em aberto com histórico. */}
        {q.length>=3&&(carregandoFin||finalizadosAchados.length>0)&&(
          <div style={{marginBottom:16}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              <span style={{...F.title,fontSize:12,fontWeight:800,color:C.gray600,letterSpacing:"0.06em"}}>
                JÁ FATURADOS
              </span>
              {carregandoFin
                ? <span style={{...F.body,fontSize:12,color:C.gray400}}>procurando...</span>
                : <span style={{...F.body,fontSize:11,fontWeight:700,color:C.white,background:C.gray500,borderRadius:10,padding:"1px 8px"}}>
                    {finalizadosAchados.length}
                  </span>}
              <span style={{...F.body,fontSize:12,color:C.gray400}}>— fora da carteira em aberto</span>
            </div>
            {finalizadosAchados.length>0&&(
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>
                {finalizadosAchados.map(o=>(
                  <OCard key={"fin-"+(o.id||"")} order={o} onClick={()=>onOpen(o)} slaCfg={slaCfg}/>
                ))}
              </div>
            )}
          </div>
        )}
        {listaSituacao.length===0?<div style={{...F.body,color:C.gray400,fontSize:14,textAlign:"center",padding:40,background:C.white,borderRadius:8,border:`1px solid ${C.gray200}`}}>{q.length>=3&&(finalizadosAchados.length>0||ocorrenciasAchadas.length>0)
            ? "Nenhum pedido em aberto com essa busca — veja os resultados acima."
            : "Nenhum pedido nesta situação."}</div>
        :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>
...
function SLAConfig({slaCfg,onSave,user}){
  ...
  // Bonificação conta em dias ÚTEIS — processo apartado e rápido.
  const[prazoBonif,setPrazoBonif]=useState(3);
  ...
  useEffect(()=>{
    apiFetch("/config-sla").then(r=>{
      if(r.success&&r.config){
        ...
        if(r.config.prazoBonificacao!=null)setPrazoBonif(r.config.prazoBonificacao);
  ...
  const save=async()=>{
    setSaving(true);
    try{
      const payload={etapas:local,prazoComBordado:Number(prazoCom),prazoSemBordado:Number(prazoSem),prazoBonificacao:Number(prazoBonif),capacidade:{interno:Number(cap.interno||0),bordadel:Number(cap.bordadel||0),mg_bordados:Number(cap.mg_bordados||0),outros:Number(cap.outros||0)},feriados,emitente:emit};
    ...
    <div style={{padding:24,display:"flex",flexDirection:"column",gap:18}}>
      ...
      <Card>
        ...
        <div style={{display:"flex",gap:24,flexWrap:"wrap"}}>
          ...
          <div>
            <label style={{...F.body,fontSize:11,fontWeight:700,color:C.gray600,textTransform:"uppercase",letterSpacing:"0.06em",display:"block",marginBottom:8}}>Bonificação</label>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input type="number" value={prazoBonif} onChange={e=>setPrazoBonif(e.target.value)}
                style={{width:80,border:`1.5px solid ${C.gray200}`,borderRadius:6,padding:"8px 12px",...F.body,fontSize:14,fontWeight:700,outline:"none",textAlign:"center"}}/>
              {/* Aqui é dia ÚTIL, diferente dos dois acima — o rótulo precisa
                  deixar isso claro, senão vira erro de leitura. */}
              <span style={{...F.body,fontSize:12,color:C.gray500,fontWeight:600}}>dias úteis</span>
            </div>
            <div style={{...F.body,fontSize:11,color:C.gray400,marginTop:4}}>Contados da emissão, pulando fim de semana e feriados.</div>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
        <CorrigirFlagBordadoBtn/>
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
        <CorrigirPrazoComBordadoBtn/>
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
        <CriarPosvendaBtn user={user}/>
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
        <PrazoBonificacaoBtn/>
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
        <SincronizarCanceladosBtn/>
        <div style={{height:1,background:C.gray100,margin:"18px 0"}}/>
...
// Corretores de dado do HubSpot que até agora só existiam como endpoint —
// dependiam de alguém chamar por fora. Aqui viram botão: consulta primeiro,
// mostra a lista e só executa depois de confirmar.
// Coloca no fluxo um pedido que a automação do HubSpot deixou de fora. Sem o
// pós-venda o pedido não existe pro SGP — e não existe erro nenhum avisando.
function PrazoBonificacaoBtn(){
  return (
    <div>
      <div style={{...F.body,fontSize:13,color:C.gray500,marginBottom:10}}>
        <strong>Prazo das bonificações.</strong> Recalcula o vencimento dos pedidos que estão na caixa
        de Bonificações usando o prazo em <em>dias úteis</em> configurado acima, contado da emissão
        (pula fim de semana e feriados). Pedido com data especial não é tocado — prazo combinado à mão
        vale mais que a regra. As bonificações novas já nascem com esse prazo, sem precisar rodar isto.
      </div>
      <MiniAcaoAdmin
        rota="/admin/prazo-bonificacao"
        rotulo="Ver prazos a ajustar"
        rotuloExec={(n)=>`Ajustar ${n} pedido(s)`}
        vazio="Todos os prazos já estão certos."
        linha={(p)=>`${p.linx?"PED - "+p.linx:p.vendasId} · ${p.cliente||"—"} — emissão ${p.emissao} · ${p.prazoAtual} → ${p.prazoNovo}`}
      />
    </div>
  );
}

function CriarPosvendaBtn({user}){
  ...
  const consultar=async()=>{
    const vid=id.replace(/\D/g,"").trim();
    if(!vid){setMsg({tipo:"erro",txt:"Informe o ID do negócio de Vendas (o número do pedido no HubSpot)."});return;}
    setBusy(true);setMsg(null);setPrevia(null);
    try{
      const r=await apiFetch(`/admin/criar-posvenda/${vid}`,"POST",{});
      if(r&&r.success)setPrevia(r); else setMsg({tipo:"erro",txt:r?.error||"Não foi possível consultar."});
    }catch(e){setMsg({tipo:"erro",txt:e.message});}
    finally{setBusy(false);}
  };
  const criar=async()=>{
    if(!confirm(`Colocar o pedido ${previa.linx?"PED - "+previa.linx:previa.vendasId} no fluxo?\n\nSerá criado: ${previa.criaraPosvenda?"pós-venda":""}${previa.criaraBordado?" + bordado":""}${previa.itensACriar?` + ${previa.itensACriar} item(ns)`:""}.`))return;
    setBusy(true);setMsg(null);
    try{
      const r=await apiFetch(`/admin/criar-posvenda/${previa.vendasId}`,"POST",
        {executar:true,ctx:{executor:user?.nome||user?.email||"Admin SGP"}});
      if(r&&r.success){
        setMsg({tipo:"ok",txt:`Pronto. Pedido no fluxo${r.itensCriados?` com ${r.itensCriados} item(ns)`:""}. Ele aparece em Em Separação no próximo carregamento do quadro.`});
        setPrevia(null);setId("");
      } else setMsg({tipo:"erro",txt:r?.error||"Não foi possível criar."});
    }catch(e){setMsg({tipo:"erro",txt:e.message});}
    finally{setBusy(false);}
  };
  ...
  return (
    <div>
      <div style={{...F.body,fontSize:13,color:C.gray500,marginBottom:10}}>
        <strong>Pedido que não entrou no fluxo.</strong> Quando a automação do HubSpot não gera o
        pós-venda, o pedido não aparece no SGP e nada avisa. Informe o ID do negócio de Vendas
        (o número grande, não o do Linx) para criar o pós-venda, o bordado e os itens.
      </div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
        <input value={id} onChange={e=>setId(e.target.value)} placeholder="ID do negócio de Vendas"
          style={{...F.body,fontSize:13,padding:"9px 12px",borderRadius:7,border:`1.5px solid ${C.gray200}`,outline:"none",width:230}}/>
        <Btn label={busy?"Consultando...":"Consultar"} icon="search" variant="secondary" disabled={busy} onClick={consultar}/>
      </div>
      {previa&&(
        <div style={{marginTop:12,border:`1px solid ${C.gray200}`,borderRadius:7,padding:"12px 14px"}}>
          <div style={{...F.body,fontSize:13,fontWeight:700,color:C.black,marginBottom:6}}>
            {previa.linx?"PED - "+previa.linx+" | ":""}{previa.vendasId} · {previa.cliente||"—"}
          </div>
          <div style={{...F.body,fontSize:12,color:C.gray600,lineHeight:1.7}}>
            <div>Natureza: <strong>{previa.natureza||"—"}</strong> · Valor: <strong>{fmtR(previa.valor||0)}</strong></div>
            <div>Pós-venda: {previa.criaraPosvenda?<strong style={{color:C.green}}>será criado</strong>:<span>já existe ({previa.posvendaExistente})</span>}</div>
            <div>Bordado: {previa.criaraBordado?<strong style={{color:C.green}}>será criado</strong>:(previa.bordadoExistente?`já existe (${previa.bordadoExistente})`:"não tem")}</div>
            <div>Itens: {previa.itensExistentes?"já existem":(previa.itensACriar?<strong style={{color:C.green}}>{previa.itensACriar} a criar</strong>:"nenhum produto no negócio")}</div>
          </div>
          {(previa.produtos||[]).length>0&&(
            <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${C.gray100}`}}>
              {previa.produtos.map((x,i)=><div key={i} style={{...F.body,fontSize:12,color:C.gray600,padding:"2px 0"}}>• {x}</div>)}
            </div>
          )}
          {previa.aviso&&(
            <div style={{marginTop:10,...F.body,fontSize:12,color:"#8a5a00",background:C.amber+"12",border:`1px solid ${C.amber}44`,borderRadius:6,padding:"8px 10px"}}>
              {previa.aviso}
            </div>
          )}
          {/* Grade que não fecha com a quantidade do produto: erro de digitação
              no pedido. Mostra e deixa a decisão com quem conhece o pedido —
              não dá pra adivinhar qual dos dois números está certo. */}
          {(previa.divergenciaGrade||[]).length>0&&(
            <div style={{marginTop:10,...F.body,fontSize:12,color:C.red,background:C.red+"0e",border:`1px solid ${C.red}44`,borderRadius:6,padding:"8px 10px"}}>
              <strong>A grade não fecha com a quantidade do produto:</strong>
              {previa.divergenciaGrade.map((d,i)=><div key={i} style={{marginTop:3}}>• {d}</div>)}
              <div style={{marginTop:5}}>Confira o pedido no HubSpot antes de criar.</div>
            </div>
          )}
          {(previa.criaraPosvenda||previa.criaraBordado||previa.itensACriar>0)&&(
            <div style={{marginTop:12}}>
              <Btn label={busy?"Criando...":"Colocar no fluxo"} icon="check" variant="success" disabled={busy} onClick={criar}/>
            </div>
          )}
        </div>
      )}
      {msg&&(
        <div style={{marginTop:10,...F.body,fontSize:12.5,fontWeight:600,padding:"9px 12px",borderRadius:7,
          background:msg.tipo==="ok"?"#ecfdf5":C.red+"0e",
          border:`1px solid ${msg.tipo==="ok"?"#04785744":C.red+"44"}`,
          color:msg.tipo==="ok"?"#047857":C.red}}>{msg.txt}</div>
      )}
    </div>
  );
}

function CorrigirPrazoComBordadoBtn(){
  return (
    <div>
      <div style={{...F.body,fontSize:13,color:C.gray500,marginBottom:10}}>
        <strong>Prazo de "sem bordado" em pedido COM bordado.</strong> A rotina noturna gravava
        emissão + prazo curto quando não conseguia confirmar se o pedido tinha bordado — e o pedido
        nascia vencido. Isto corrige: com a amostra já aprovada, recalcula pela data da aprovação;
        se ainda não foi aprovada, apaga o prazo e o card volta a dizer "aguardando amostra".
        Pedido com data especial não é tocado.
      </div>
      <MiniAcaoAdmin
        rota="/admin/corrigir-prazo-com-bordado"
        rotulo="Ver pedidos com prazo errado"
        rotuloExec={(n)=>`Corrigir ${n} pedido(s)`}
        vazio="Nenhum pedido com prazo errado — está tudo certo."
        linha={(p)=>`${p.linx?"PED - "+p.linx:p.vendasId} · ${p.cliente||"—"} — ${String(p.prazoErrado||"").slice(0,10)} → ${p.acao==="apagar"?"apagar (aguardando amostra)":String(p.novoPrazo||"").slice(0,10)}`}
      />
    </div>
  );
}

function CorrigirFlagBordadoBtn(){
  return (
    <div>
      <div style={{...F.body,fontSize:13,color:C.gray500,marginBottom:10}}>
        <strong>Flag de bordado errada.</strong> Pedidos que têm negócio de Bordado <em>ativo</em> mas
        estão com <code>pedido_com_bordado</code> vazio ou false no HubSpot. É o que faz o pedido
        aparecer como "sem bordado" e ganhar prazo de pedido sem bordado — nascendo vencido.
        Só considera bordado ativo: pedido corrigido para silk/DTF não é remarcado.
      </div>
      <MiniAcaoAdmin
        rota="/admin/corrigir-flag-bordado"
        rotulo="Ver pedidos com flag errada"
        rotuloExec={(n)=>`Corrigir ${n} pedido(s)`}
        vazio="Nenhum pedido com flag errada — está tudo certo."
        linha={(p)=>`${p.linx?"PED - "+p.linx:p.vendasId} · ${p.cliente||"—"} — flag: ${p.flagAtual}`}
      />
    </div>
  );
}

function SincronizarCanceladosBtn(){
  return (
    <div>
      <div style={{...F.body,fontSize:13,color:C.gray500,marginBottom:10}}>
        <strong>Pedidos cancelados com filhos ativos.</strong> Cancelar no funil de Vendas não mexe no
        Pós-Venda nem no Bordado — eles ficam abertos no HubSpot para sempre. O SGP já esconde esses
        pedidos do quadro; isto arruma o dado, levando o pós-venda para Cancelamento e finalizando o bordado.
      </div>
      <MiniAcaoAdmin
        rota="/admin/sincronizar-cancelados"
        rotulo="Ver negócios a encerrar"
        rotuloExec={(n)=>`Encerrar ${n} negócio(s)`}
        vazio="Nenhum negócio pendente — está tudo certo."
        linha={(p)=>`${p.linx?"PED - "+p.linx:p.vendasId} · ${p.nome||"—"} — ${p.tipo} em ${p.etapaAtual}`}
      />
    </div>
  );
}
...
// ─── EXECUTAR: PENDENTE PAGAMENTO ────────────────────────────────────────────
// Etapa de cobrança: o pedido está embalado e só não fatura porque o ERP ainda
// não confirmou o pagamento. Antes as ações existiam SÓ nos botões da fila —
// quem abria o pedido pra conferir valor e condição ficava sem saída e tinha
// que voltar pra lista. Aqui ficam as três coisas que se faz nesta etapa:
// ver o status do pagamento, registrar a cobrança e liberar mesmo assim.
function PendentePagamentoAcao({order,me,setActionDone}){
  const [hist,setHist]=useState(null);
  const [texto,setTexto]=useState("");
  const [busy,setBusy]=useState(false);
  const [msg,setMsg]=useState(null);
  const pvId=order?.posvendaId;

  useEffect(()=>{
    if(!pvId){setHist([]);return;}
    apiFetch(`/pendente-pagamento-tratativas/${pvId}`)
      .then(r=>setHist(r&&r.success?(r.data||[]):[]))
      .catch(()=>setHist([]));
  },[pvId]);

  const registrar=async()=>{
    const t=texto.trim();
    if(!t){setMsg({tipo:"erro",txt:"Escreva o que foi combinado com o cliente."});return;}
    setBusy("tratativa");setMsg(null);
    try{
      const r=await apiFetch(`/pendente-pagamento-tratativa/${pvId}`,"POST",
        {tratativa:t,ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
      if(r&&r.success){
        setTexto("");
        setHist(p=>[{texto:"[SGP] [COBRANÇA] "+t,em:new Date().toISOString()},...(p||[])]);
        setMsg({tipo:"ok",txt:"Cobrança registrada na timeline do pedido."});
      } else setMsg({tipo:"erro",txt:r?.error||"Não foi possível registrar."});
    }catch(e){setMsg({tipo:"erro",txt:e.message});}
    finally{setBusy(false);}
  };

  const liberar=async()=>{
    const motivo=prompt(`Liberar o pedido ${order.pedidoLinx||order.vendasId} para Análise de Frete SEM a confirmação de pagamento do ERP?\n\nDescreva o motivo (obrigatório — fica registrado na timeline):`);
    if(motivo===null)return;
    if(!String(motivo).trim()){alert("Motivo obrigatório.");return;}
    setBusy("liberar");setMsg(null);
    try{
      const r=await apiFetch(`/pendente-pagamento-liberar/${pvId}`,"POST",
        {motivo:String(motivo).trim(),ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
      if(r&&r.success){
        setMsg({tipo:"ok",txt:"Pedido liberado — seguiu para Análise de Frete."});
        setActionDone&&setActionDone(true);
        recarregarAposAcao(order);
      } else setMsg({tipo:"erro",txt:r?.error||"Não foi possível liberar."});
    }catch(e){setMsg({tipo:"erro",txt:e.message});}
    finally{setBusy(false);}
  };

  const liberado=order.pagamentoLiberado===true;
  const box={border:`1.5px solid ${C.gray200}`,borderRadius:8,padding:"14px 16px"};
  return (
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
      {/* Status do pagamento — é o que decide se o pedido sai daqui sozinho */}
      {liberado
        ? <div style={{background:C.green+"12",border:"1.5px solid #06534644",borderRadius:8,padding:"12px 14px",display:"flex",gap:10,alignItems:"center"}}>
            <Ic n="check" s={20} c="#065f46"/>
            <div>
              <div style={{...F.title,fontSize:12,fontWeight:800,color:"#065f46",letterSpacing:"0.04em"}}>PAGAMENTO LIBERADO</div>
              <div style={{...F.body,fontSize:12,color:C.gray600,marginTop:2}}>O ERP já confirmou. O pedido sai desta caixa sozinho na próxima verificação — não precisa fazer nada.</div>
            </div>
          </div>
        : <div style={{background:C.red+"0e",border:`1.5px solid ${C.red}55`,borderRadius:8,padding:"12px 14px",display:"flex",gap:10,alignItems:"center"}}>
            <Ic n="warn" s={20} c={C.red}/>
            <div>
              <div style={{...F.title,fontSize:12,fontWeight:800,color:C.red,letterSpacing:"0.04em"}}>PAGAMENTO PENDENTE</div>
              <div style={{...F.body,fontSize:12,color:C.gray600,marginTop:2}}>O pedido está embalado e retido até o ERP confirmar o pagamento.</div>
            </div>
          </div>}

      {/* O que o cobrador precisa ter à mão pra ligar pro cliente */}
      <div style={{...box,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12}}>
        {[["Valor do pedido",fmtR(order.valor||0)],
          ["Condição de pagamento",order.condicaoPagamento||"—"],
          ["Cliente",order.client||order.razaoSocial||"—"],
          ["Vendedor",order.vendedor||"—"]].map(([k,v])=>(
          <div key={k}>
            <div style={{...F.body,fontSize:10,fontWeight:800,color:C.gray500,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:3}}>{k}</div>
            <div style={{...F.body,fontSize:13,fontWeight:700,color:C.black,wordBreak:"break-word"}}>{v}</div>
          </div>
        ))}
      </div>
      ...
      {/* Registrar cobrança */}
      <div style={box}>
        <div style={{...F.title,fontSize:11,fontWeight:800,color:C.gray600,letterSpacing:"0.07em",marginBottom:8}}>REGISTRAR COBRANÇA</div>
        <div style={{...F.body,fontSize:12,color:C.gray500,marginBottom:8}}>
          O que foi combinado com o cliente. Fica na timeline do pedido — a próxima pessoa que abrir vê o histórico.
        </div>
        <textarea value={texto} onChange={e=>setTexto(e.target.value)} rows={3}
          placeholder="Ex.: Falei com o financeiro, o pagamento entra na quinta."
          style={{width:"100%",...F.body,fontSize:13,border:`1px solid ${C.gray200}`,borderRadius:6,padding:"10px 12px",outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
        <div style={{marginTop:10}}>
          <Btn label={busy==="tratativa"?"Registrando...":"Registrar cobrança"} icon="check"
               variant="secondary" disabled={!!busy} onClick={registrar}/>
        </div>
        {hist===null
          ? <div style={{...F.body,fontSize:12,color:C.gray400,marginTop:10}}>Carregando histórico...</div>
          : hist.length>0&&(
            <div style={{marginTop:12,borderTop:`1px solid ${C.gray100}`,paddingTop:10,display:"flex",flexDirection:"column",gap:7}}>
              <div style={{...F.body,fontSize:10,fontWeight:800,color:C.gray500,textTransform:"uppercase",letterSpacing:"0.06em"}}>
                Histórico ({hist.length})
              </div>
              {hist.map((h,i)=>(
                <div key={i} style={{...F.body,fontSize:12.5,color:C.gray700,background:C.gray50,borderRadius:6,padding:"8px 10px"}}>
                  <div style={{whiteSpace:"pre-wrap",wordBreak:"break-word"}}>{String(h.texto||"").replace(/^\[SGP\]\s*\[COBRANÇA\]\s*/,"")}</div>
                  {h.em&&<div style={{fontSize:11,color:C.gray400,marginTop:3}}>{fmtD(h.em)}</div>}
                </div>
              ))}
            </div>
          )}
      ...
      {/* Liberar mesmo assim — sai do fluxo normal, por isso fica por último */}
      <div style={{...box,borderColor:C.red+"44"}}>
        <div style={{...F.title,fontSize:11,fontWeight:800,color:C.red,letterSpacing:"0.07em",marginBottom:6}}>LIBERAR MESMO ASSIM</div>
        <div style={{...F.body,fontSize:12,color:C.gray600,marginBottom:10,lineHeight:1.5}}>
          Manda o pedido para a Análise de Frete <strong>sem</strong> a confirmação de pagamento do ERP.
          Use quando a liberação foi combinada por fora (diretoria, acordo com o cliente). O motivo é obrigatório e fica registrado.
        </div>
        <Btn label={busy==="liberar"?"Liberando...":"Liberar para Análise de Frete"} icon="send"
             variant="primary" disabled={!!busy} onClick={liberar}/>
      </div>

      {msg&&(
        <div style={{...F.body,fontSize:12.5,fontWeight:600,padding:"9px 12px",borderRadius:7,
          background:msg.tipo==="ok"?"#ecfdf5":C.red+"0e",
          border:`1px solid ${msg.tipo==="ok"?"#04785744":C.red+"44"}`,
          color:msg.tipo==="ok"?"#047857":C.red}}>{msg.txt}</div>
      )}
...
// Marca a aplicação de silk/DTF como pronta. Não move o pedido de etapa —
// silk/DTF é personalização da peça, não fase do pedido. O que ele faz é
// destravar o "Conferido" na Conferência e Direcionamento.
function SilkDtfConcluirBtn({order,me,onOk,size}){
  const [busy,setBusy]=useState(false);
  const clicar=async()=>{
    if(!order?.posvendaId){alert("Pedido sem negócio de Pós-venda.");return;}
    if(!confirm(`Confirmar que o silk/DTF do pedido ${order.pedidoLinx||order.vendasId} está aplicado?\n\nO pedido fica liberado para a conferência seguir para a Expedição.`))return;
    setBusy(true);
    try{
      const r=await apiFetch("/silk-dtf-concluido/"+order.posvendaId,"POST",
        {ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
      if(r&&r.success){ onOk?onOk(r):recarregarAposAcao(order); }
      else alert("Erro: "+(r?.error||"desconhecido"));
    }catch(e){alert("Erro: "+e.message);}
    finally{setBusy(false);}
  };
  return <Btn label={busy?"Marcando...":"Silk/DTF aplicado"} icon="check" variant="primary"
              size={size||"sm"} disabled={busy} onClick={clicar}/>;
}

// ═══ [FRETE] VOLUMES — PREVISTO (cotação) × REALIZADO (expedição) ═══════════
// O TMS cotado no HubSpot consolida o pedido em caixas e grava o PREVISTO.
// A expedição embala de verdade e nem sempre fecha no previsto — por isso ela
// registra o REALIZADO, e é ESSE que vale pra contratar a transportadora.
// Duas telas usam isto: Expedição (registra) e Análise de Frete (só confere).
const CAIXAS_FRETE_SGP = ["Caixa P","Caixa M","Caixa G","Caixa GG","Saco PP","Saco P","Saco M","Saco G"];

// Lista de "2x Caixa M" em chips. `itens` (quantas peças o TMS pôs na caixa)
// só existe no previsto.
function ChipsVolumes({itens,cor,vazio}){
  if(!itens||!itens.length) return <span style={{...F.body,fontSize:12.5,color:C.gray400}}>{vazio||"—"}</span>;
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
      {itens.map((v,i)=>(
        <span key={i} style={{display:"inline-flex",alignItems:"baseline",gap:4,background:cor+"14",border:`1px solid ${cor}44`,
          borderRadius:6,padding:"5px 10px",...F.body,fontSize:12.5,fontWeight:700,color:cor}}>
          {v.amount}× {v.box}
          {v.itens!=null&&<span style={{fontSize:11,fontWeight:500,color:C.gray600}}>({v.itens} {v.itens===1?"item":"itens"})</span>}
        </span>
      ))}
    </div>
  );
...
// Bloco só de leitura. Usado na Análise de Frete, onde a decisão é contratar o
// frete — então o realizado tem que estar impossível de confundir com o previsto.
function VolumesComparativo({fv,compacto}){
  const f=fv||{};
  const prev=f.previsto||{}, real=f.realizado||{};
  // ⚠ NÃO esconder quando está tudo vazio. Bloco ausente é ambíguo: quem olha
  // não sabe se o pedido não tem cotação ou se a tela quebrou. Mostra o estado
  // vazio dizendo qual é o caso.
  const vazio=!f.temPrevisto&&!f.temRealizado;
  if(vazio) return (
    <div style={{border:`1.5px dashed ${C.gray200}`,borderRadius:8,padding:"12px 14px"}}>
      <div style={{...F.title,fontSize:11,fontWeight:800,letterSpacing:"0.07em",color:C.gray500,marginBottom:4}}>📦 VOLUMES DO FRETE</div>
      <div style={{...F.body,fontSize:12.5,color:C.gray500}}>
        Este pedido não tem cotação de volumes. Só aparece aqui quando a cotação foi feita pelo TMS no HubSpot.
      </div>
    </div>
  );
  return (
    <div style={{border:`1.5px solid ${f.divergente?C.red+"66":C.gray200}`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:f.divergente?C.red+"0e":C.gray50,padding:"9px 14px",borderBottom:`1px solid ${C.gray200}`,
        display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
        <span style={{...F.title,fontSize:11,fontWeight:800,letterSpacing:"0.07em",color:f.divergente?C.red:C.gray700}}>
          📦 VOLUMES DO FRETE
        </span>
        {f.divergente&&<span style={{...F.body,fontSize:11,fontWeight:800,color:C.white,background:C.red,borderRadius:4,padding:"2px 8px"}}>DIVERGENTE</span>}
        {f.pendente&&<span style={{...F.body,fontSize:11,fontWeight:800,color:"#92400e",background:"#fef3c7",border:"1px solid #f59e0b66",borderRadius:4,padding:"2px 8px"}}>AGUARDANDO A EXPEDIÇÃO</span>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:compacto?"1fr":"repeat(auto-fit,minmax(260px,1fr))",gap:0}}>
        <div style={{padding:"12px 14px",borderRight:compacto?"none":`1px solid ${C.gray100}`,borderBottom:compacto?`1px solid ${C.gray100}`:"none"}}>
          <div style={{...F.body,fontSize:10,fontWeight:800,color:C.gray500,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>
            Previsto na cotação {prev.qtd>0&&`· ${prev.qtd} volume${prev.qtd!==1?"s":""}`}
          </div>
          <ChipsVolumes itens={prev.itens} cor={C.gray600} vazio="Sem cotação de volumes"/>
        </div>
        <div style={{padding:"12px 14px",background:f.temRealizado?"#ecfdf5":C.white}}>
          <div style={{...F.body,fontSize:10,fontWeight:800,color:f.temRealizado?"#047857":C.gray500,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>
            Realizado pela expedição {real.qtd>0&&`· ${real.qtd} volume${real.qtd!==1?"s":""}`}
          </div>
          <ChipsVolumes itens={real.itens} cor="#047857" vazio="A expedição ainda não registrou"/>
        </div>
      </div>
      {f.divergente&&(
        <div style={{...F.body,fontSize:12,color:C.red,background:C.red+"08",padding:"9px 14px",borderTop:`1px solid ${C.red}33`,fontWeight:600}}>
          A expedição consolidou diferente do cotado. Contrate a transportadora pelo <strong>realizado</strong> — o valor cotado pode mudar.
        </div>
      )}
    </div>
  );
...
// Bloco de REGISTRO — só na Expedição. Duas saídas: confirmar o previsto num
// clique (caso comum) ou informar as caixas de fato usadas.
function VolumesExpedicao({order,user,onSalvo}){
  const fv=order?.freteVolumes||{};
  const [linhas,setLinhas]=useState(()=>{
    const jaTem=(fv.realizado&&fv.realizado.itens)||[];
    if(jaTem.length) return jaTem.map(v=>({box:v.box,amount:String(v.amount),weight:""}));
    return [{box:"",amount:"1",weight:""}];
  });
  const [salvando,setSalvando]=useState(false);
  const [msg,setMsg]=useState(null);
  const [editando,setEditando]=useState(!fv.temRealizado);

  const enviar=async(payload,textoOk)=>{
    setSalvando(true);setMsg(null);
    try{
      const r=await apiFetch(`/volumes-expedicao/${order.vendasId}`,"POST",
        {...payload,ctx:{executor:user?.nome||user?.email||"Usuário SGP"}});
      if(r&&r.success){
        setMsg({tipo:"ok",txt:textoOk||`Registrado: ${r.texto} (${r.total} volume${r.total!==1?"s":""}).`});
        setEditando(false);
        onSalvo&&onSalvo(r);
      } else setMsg({tipo:"erro",txt:r?.error||"Não foi possível registrar."});
    }catch(e){setMsg({tipo:"erro",txt:e.message});}
  ...
  const linhaSet=(i,campo,val)=>setLinhas(p=>p.map((l,ix)=>ix===i?{...l,[campo]:val}:l));
  const addLinha=()=>setLinhas(p=>[...p,{box:"",amount:"1",weight:""}]);
  const delLinha=(i)=>setLinhas(p=>p.length===1?p:p.filter((_,ix)=>ix!==i));
  const salvarManual=()=>{
    const boxes=linhas.filter(l=>l.box&&Number(l.amount)>0)
      .map(l=>({box:l.box,amount:Number(l.amount),...(Number(l.weight)>0?{weight:Number(l.weight)}:{})}));
    if(!boxes.length){setMsg({tipo:"erro",txt:"Escolha o tipo de caixa e a quantidade de pelo menos um volume."});return;}
    enviar({boxes});
  };

  const inp={...F.body,fontSize:13,padding:"8px 10px",borderRadius:7,border:`1.5px solid ${C.gray200}`,outline:"none",boxSizing:"border-box"};
  return (
    <div style={{border:`1.5px solid ${C.gray200}`,borderRadius:8,overflow:"hidden"}}>
      <div style={{background:C.gray50,padding:"9px 14px",borderBottom:`1px solid ${C.gray200}`}}>
        <span style={{...F.title,fontSize:11,fontWeight:800,letterSpacing:"0.07em",color:C.gray700}}>📦 CONSOLIDAÇÃO EM CAIXAS</span>
      </div>
      <div style={{padding:14,display:"flex",flexDirection:"column",gap:12}}>
        <div>
          <div style={{...F.body,fontSize:10,fontWeight:800,color:C.gray500,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>
            Previsto na cotação {fv.previsto?.qtd>0&&`· ${fv.previsto.qtd} volume${fv.previsto.qtd!==1?"s":""}`}
          </div>
          <ChipsVolumes itens={fv.previsto?.itens} cor={C.gray600} vazio="Este pedido não tem cotação de volumes."/>
        </div>
        {fv.temRealizado&&!editando&&(
          <div style={{background:"#ecfdf5",border:"1.5px solid #04785744",borderRadius:8,padding:"12px 14px"}}>
            <div style={{...F.body,fontSize:10,fontWeight:800,color:"#047857",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>
              Registrado · {fv.realizado.qtd} volume{fv.realizado.qtd!==1?"s":""}
            </div>
            <ChipsVolumes itens={fv.realizado.itens} cor="#047857"/>
            <div style={{marginTop:10}}>
              <Btn label="Corrigir" variant="secondary" size="sm" onClick={()=>setEditando(true)}/>
            </div>
          </div>
        )}
        {editando&&(<>
          {fv.temPrevisto&&(
            <Btn label={salvando?"Registrando...":"Consolidei conforme o previsto"} icon="check" variant="success"
                 disabled={salvando} onClick={()=>enviar({conformePrevisto:true},"Confirmado conforme o previsto.")}/>
          )}
          <div style={{...F.body,fontSize:12,color:C.gray500}}>
            {fv.temPrevisto?"Não fechou no previsto? Informe abaixo o que foi usado de fato:":"Informe as caixas usadas:"}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {linhas.map((l,i)=>(
              <div key={i} style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                <select value={l.box} onChange={e=>linhaSet(i,"box",e.target.value)} style={{...inp,flex:"1 1 150px",cursor:"pointer"}}>
                  <option value="">Tipo de caixa...</option>
                  {CAIXAS_FRETE_SGP.map(c=><option key={c} value={c}>{c}</option>)}
                </select>
                <input type="number" min="1" value={l.amount} onChange={e=>linhaSet(i,"amount",e.target.value)}
                       placeholder="Qtd" style={{...inp,width:78}}/>
                <input type="number" min="0" step="0.1" value={l.weight} onChange={e=>linhaSet(i,"weight",e.target.value)}
                       placeholder="Peso kg" title="Opcional — só se pesou" style={{...inp,width:100}}/>
                <button onClick={()=>delLinha(i)} disabled={linhas.length===1} title="Remover"
                  style={{background:"none",border:"none",cursor:linhas.length===1?"default":"pointer",padding:6,opacity:linhas.length===1?0.3:1}}>
                  <Ic n="close" s={15} c={C.gray500}/>
                </button>
              </div>
            ))}
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <Btn label="+ Outro tipo de caixa" variant="ghost" size="sm" onClick={addLinha}/>
            <Btn label={salvando?"Salvando...":"Registrar volumes"} icon="check" variant="primary"
                 disabled={salvando} onClick={salvarManual}/>
            {fv.temRealizado&&<Btn label="Cancelar" variant="secondary" size="sm" onClick={()=>{setEditando(false);setMsg(null);}}/>}
          </div>
        </>)}
        {msg&&(
          <div style={{...F.body,fontSize:12.5,fontWeight:600,padding:"9px 12px",borderRadius:7,
...
// Visão tabular de tudo que está em aberto: uma linha por pedido, ordenável por
// qualquer coluna, com busca, filtros e exportação pro Excel. Existe porque as
// telas de fila respondem "o que eu faço agora"; esta responde "como está a
// carteira inteira" — é relatório, não fila de trabalho.
// Fonte: o mesmo snapshot das demais telas (sem chamada extra ao HubSpot).
function RelatorioPedidos({onOpen}){
  const snap=useSnapshotAberto();
  const loading=snap.loading&&!snap.data;
  const [busca,setBusca]=useState("");
  const [fRep,setFRep]=useState("todos");
  const [fEtapa,setFEtapa]=useState("todos");
  const [fSit,setFSit]=useState("todos");           // todos | atraso | prazo
  const [ord,setOrd]=useState({col:"emissao",dir:"desc"});

  // Contagem única por pedido: o mesmo card pode estar em duas etapas
  // (separação + programação, por exemplo) e contaria dobrado.
  const base=useMemo(()=>{
    const vistos=new Set(); const out=[];
    for(const o of snapTodosPedidos(snap.data)){
      const k=String(o.vendasId||o.id);
      if(vistos.has(k))continue;
      vistos.add(k);
      out.push(normalizarCard(o,o.etapa));
    }
    return out;
  },[snap.data]);

  const agora=Date.now();
  const linhas=useMemo(()=>base.map(o=>{
    const venc=dataVencimento(o);
    return {
      o,
      linx:o.pedidoLinx||"",
      mercos:o.vendasId||"",
      emissao:o.dataFechamento||o.entradaAt||null,
      cliente:o.client||o.razaoSocial||"",
      rep:o.vendedor||"",
      valor:Number(o.valor||0),
      qtde:pecasDoCard(o),
      etapa:o.etapa||"",
      venc,
      atrasado:!!(venc&&venceuAntes(venc,agora)),
      especial:temDataEspecial(o),
    };
  }),[base]);

  const representantes=useMemo(()=>
    [...new Set(linhas.map(l=>l.rep).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"pt-BR")),[linhas]);
  const etapas=useMemo(()=>
    [...new Set(linhas.map(l=>l.etapa).filter(Boolean))].sort((a,b)=>a.localeCompare(b,"pt-BR")),[linhas]);

  const filtradas=useMemo(()=>{
    const q=semAcento(busca.trim());
    return linhas.filter(l=>{
      if(fRep!=="todos"&&l.rep!==fRep)return false;
      if(fEtapa!=="todos"&&l.etapa!==fEtapa)return false;
      if(fSit==="atraso"&&!l.atrasado)return false;
      if(fSit==="prazo"&&l.atrasado)return false;
      if(!q)return true;
      return semAcento(`${l.linx} ${l.mercos} ${l.cliente} ${l.rep} ${l.etapa}`).includes(q);
    });
  },[linhas,busca,fRep,fEtapa,fSit]);

  const ordenadas=useMemo(()=>{
    const sinal=ord.dir==="asc"?1:-1;
    const val=(l)=>{
      switch(ord.col){
        case "linx":    return Number(l.linx)||l.linx||"";
        case "mercos":  return l.mercos;
        case "emissao": return l.emissao?new Date(l.emissao).getTime():0;
        case "cliente": return l.cliente;
        case "rep":     return l.rep;
        case "valor":   return l.valor;
        case "qtde":    return l.qtde;
        case "etapa":   return l.etapa;
        case "venc":    return l.venc?new Date(l.venc).getTime():Infinity;  // sem prazo por último
        default:        return 0;
      }
    };
    return [...filtradas].sort((a,a2)=>{
  ...
  // KPIs seguem o que está FILTRADO — senão o cabeçalho contradiz a tabela.
  const kpi=useMemo(()=>{
    const total=filtradas.length;
    const valor=filtradas.reduce((s,l)=>s+l.valor,0);
    const atraso=filtradas.filter(l=>l.atrasado).length;
    return {total,valor,atraso,pct:total?(atraso/total*100):0};
  },[filtradas]);

  const sort=(col)=>setOrd(p=>p.col===col?{col,dir:p.dir==="asc"?"desc":"asc"}:{col,dir:col==="valor"||col==="qtde"||col==="emissao"?"desc":"asc"});

  const exportar=()=>{
    if(!ordenadas.length){alert("Nada para exportar.");return;}
    const esc=(v)=>String(v==null?"":v).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");
    const cols=["Pedido Linx","Mercos","Emissão","Cliente","Representante","Valor (R$)","Qtde","Etapa Atual","Vencimento","Situação"];
    const corpo=ordenadas.map(l=>[
      l.linx,l.mercos,
      l.emissao?new Date(l.emissao).toLocaleDateString("pt-BR"):"",
      l.cliente,l.rep,
      l.valor.toLocaleString("pt-BR",{minimumFractionDigits:2}),
      l.qtde,l.etapa,
      l.venc?fmtVenc(l.venc):"",
      l.atrasado?"Em atraso":(l.venc?"No prazo":"Sem prazo"),
    ]);
    const thead="<tr>"+cols.map(c=>`<th style="background:#9E0B0F;color:#fff;font-weight:bold;padding:6px;border:1px solid #ccc">${esc(c)}</th>`).join("")+"</tr>";
    const tbody=corpo.map(r=>"<tr>"+r.map((c,i)=>{
      const cor=i===9?(String(c)==="Em atraso"?' color:#b91c1c;font-weight:bold;':' color:#047857;'):"";
      return `<td style="padding:5px;border:1px solid #ccc;${cor}">${esc(c)}</td>`;
    }).join("")+"</tr>").join("");
    const hoje=new Date().toLocaleDateString("pt-BR");
    const resumo=`Relatório de Pedidos em Aberto — ${hoje} · ${kpi.total} pedidos · `
      +`${fmtR(kpi.valor)} · ${kpi.atraso} em atraso (${kpi.pct.toFixed(1)}%)`;
    const html=`<html xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"></head><body>`+
      `<h3>${esc(resumo)}</h3><table border="1" cellspacing="0">${thead}${tbody}</table></body></html>`;
    const blob=new Blob(["﻿",html],{type:"application/vnd.ms-excel;charset=utf-8"});
    const a=document.createElement("a");
    a.href=URL.createObjectURL(blob);
    a.download=`pedidos-em-aberto-${new Date().toISOString().slice(0,10)}.xls`;
    document.body.appendChild(a);a.click();document.body.removeChild(a);
    setTimeout(()=>URL.revokeObjectURL(a.href),1000);
  };

  const KPI=({label,valor,cor,bg,icone})=>(
    <div style={{flex:"1 1 180px",minWidth:170,background:bg,border:`1px solid ${cor}33`,borderRadius:12,padding:"14px 16px"}}>
      <div style={{...F.body,fontSize:12,color:C.gray600,fontWeight:600,display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
        <Ic n={icone} s={13} c={cor}/> {label}
      </div>
      <div style={{...F.title,fontSize:26,fontWeight:800,color:cor,lineHeight:1}}>{valor}</div>
    </div>
  );
  const Th=({col,label,alinha})=>(
    <th onClick={()=>sort(col)} style={{...F.body,fontSize:10,fontWeight:700,color:C.gray600,textTransform:"uppercase",letterSpacing:"0.05em",
      padding:"10px 12px",textAlign:alinha||"left",cursor:"pointer",whiteSpace:"nowrap",userSelect:"none",borderBottom:`1.5px solid ${C.gray200}`}}>
      {label}{ord.col===col&&<span style={{color:C.red,marginLeft:4}}>{ord.dir==="asc"?"▲":"▼"}</span>}
    </th>
  );
  const td={...F.body,fontSize:12.5,color:C.gray700,padding:"10px 12px",borderBottom:`1px solid ${C.gray100}`,whiteSpace:"nowrap"};
  const selEstilo={...F.body,fontSize:13,padding:"9px 12px",borderRadius:8,border:`1.5px solid ${C.gray200}`,background:C.white,color:C.gray700,cursor:"pointer",minWidth:170};

  return (
    <div style={{padding:24}}>
      <PageH title="Relatório de Pedidos"
             sub={loading?"Carregando...":"Carteira em aberto — clique no título da coluna para ordenar"}
             onRefresh={snap.refresh} refreshing={snap.loading}/>
      <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:18}}>
        <KPI label="Pedidos em Aberto" valor={kpi.total} cor="#2563eb" bg="#eff6ff" icone="box"/>
        <KPI label="Valor em Aberto" valor={fmtR(kpi.valor)} cor="#059669" bg="#ecfdf5" icone="chart"/>
        <KPI label="Em Atraso" valor={kpi.atraso} cor="#dc2626" bg="#fef2f2" icone="alert"/>
        <KPI label="% Atraso" valor={kpi.pct.toFixed(1)+"%"} cor="#b45309" bg="#fffbeb" icone="clock"/>
      </div>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",marginBottom:16}}>
        <div style={{position:"relative",flex:"1 1 280px",minWidth:220}}>
          <span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",display:"flex"}}><Ic n="search" s={14} c={C.gray400}/></span>
          <input value={busca} onChange={e=>setBusca(e.target.value)}
            placeholder="Buscar pedido, Mercos, cliente, representante..."
            style={{...F.body,fontSize:13,width:"100%",padding:"10px 12px 10px 34px",borderRadius:8,border:`1.5px solid ${C.gray200}`,outline:"none",boxSizing:"border-box"}}/>
        </div>
        <select value={fRep} onChange={e=>setFRep(e.target.value)} style={selEstilo}>
          <option value="todos">Todos os representantes</option>
          {representantes.map(r=><option key={r} value={r}>{r}</option>)}
        </select>
        <select value={fEtapa} onChange={e=>setFEtapa(e.target.value)} style={selEstilo}>
          <option value="todos">Todas as etapas</option>
          {etapas.map(e=><option key={e} value={e}>{e}</option>)}
        </select>
        <select value={fSit} onChange={e=>setFSit(e.target.value)} style={selEstilo}>
          <option value="todos">No prazo e em atraso</option>
          <option value="atraso">Só em atraso</option>
          <option value="prazo">Só no prazo</option>
        </select>
        <span style={{...F.body,fontSize:12,color:C.gray500,whiteSpace:"nowrap"}}>{ordenadas.length} pedido{ordenadas.length!==1?"s":""}</span>
        <Btn label="Exportar Excel" icon="download" variant="secondary" onClick={exportar}/>
      </div>
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",minWidth:1050}}>
            <thead><tr>
              <Th col="linx" label="Pedido Linx"/><Th col="mercos" label="Mercos"/>
              <Th col="emissao" label="Emissão"/><Th col="cliente" label="Cliente"/>
              <Th col="rep" label="Representante"/><Th col="valor" label="Valor" alinha="right"/>
              <Th col="qtde" label="Qtde" alinha="right"/><Th col="etapa" label="Etapa Atual"/>
              <Th col="venc" label="Vencimento"/>
              <th style={{...F.body,fontSize:10,fontWeight:700,color:C.gray600,textTransform:"uppercase",letterSpacing:"0.05em",padding:"10px 12px",borderBottom:`1.5px solid ${C.gray200}`}}>Link</th>
            </tr></thead>
            <tbody>
              {ordenadas.map(l=>(
                <tr key={l.o.id} style={{background:l.especial?BG_ESPECIAL:C.white}}>
                  <td style={{...td,fontWeight:700,color:C.black}}>{l.linx?"#"+l.linx:"—"}</td>
                  <td style={{...td,color:C.gray500}}>{l.mercos}</td>
                  <td style={td}>{l.emissao?new Date(l.emissao).toLocaleDateString("pt-BR"):"—"}</td>
                  <td style={{...td,whiteSpace:"normal",minWidth:180}}>{l.cliente||"—"}</td>
                  <td style={td}>{l.rep||"—"}</td>
                  <td style={{...td,textAlign:"right",fontWeight:600}}>{fmtR(l.valor)}</td>
                  <td style={{...td,textAlign:"right"}}>{l.qtde||"—"}</td>
                  <td style={td}><ETag etapa={l.etapa}/></td>
                  <td style={td}>
                    {l.venc?(<>
                      <div style={{color:l.especial?COR_ESPECIAL:C.gray700,fontWeight:l.especial?700:500}}>{fmtVenc(l.venc)}</div>
                      <div style={{fontSize:11,fontWeight:700,color:l.atrasado?"#b91c1c":"#047857"}}>{l.atrasado?"Em atraso":"No prazo"}</div>
                    </>):<span style={{color:C.gray400}}>Aguardando amostra</span>}
                  </td>
                  <td style={td}>
                    <button onClick={()=>onOpen&&onOpen(l.o)}
                      style={{...F.body,fontSize:12,fontWeight:600,color:C.red,background:"none",border:"none",cursor:"pointer",padding:0}}>Abrir</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {!loading&&!ordenadas.length&&(
            <div style={{...F.body,fontSize:14,color:C.gray500,padding:"32px 16px",textAlign:"center"}}>
...
// ─── PEDIDOS SEM VENDA (auditoria) ───────────────────────────────────────────
// O quadro esconde o pedido cujo negócio de VENDAS foi excluído no HubSpot:
// sem ele não há cliente, valor, prazo nem itens de venda — o card apareceria
// em branco e atrapalharia a leitura das filas. Esta tela é a contrapartida:
// mostra o que foi escondido, com o que ainda dá pra saber, pra decidir entre
// restaurar o negócio na lixeira do HubSpot (30 dias) ou encerrar o pós-venda.
function PedidosSemVenda(){
  const [d,setD]=useState(null);
  const [loading,setLoading]=useState(true);
  const [erro,setErro]=useState("");
  const carregar=()=>{
    setLoading(true); setErro("");
    apiFetch("/admin/pedidos-sem-vendas")
      .then(r=>{ if(r.success)setD(r); else setErro(r.error||"Erro ao carregar."); })
  ...
  return (
    <div style={{padding:24}}>
      <PageH title="Pedidos sem Venda"
             sub={loading?"Carregando...":`${lista.length} pedido${lista.length!==1?"s":""} oculto${lista.length!==1?"s":""} do quadro`}
             onRefresh={carregar} refreshing={loading}/>
      {erro&&<div style={{...F.body,fontSize:13,color:C.red,marginBottom:14}}>{erro}</div>}
      <div style={{...F.body,fontSize:13,color:C.gray600,lineHeight:1.6,marginBottom:18,maxWidth:760}}>
        O negócio de Vendas destes pedidos não existe mais no HubSpot. Sem ele o SGP
        não tem cliente, valor, prazo nem itens de venda — por isso eles saem das filas
        e dos contadores. Para trazer um de volta, restaure o negócio na lixeira do
        HubSpot (disponível por 30 dias); se o pedido não deve mesmo existir, encerre
        ou exclua também o Pós-Venda e o Bordado.
      </div>
      {!loading&&!lista.length&&(
        <div style={{...F.body,fontSize:14,color:C.gray500,padding:"28px 0"}}>
          Nenhum pedido oculto. Todos os pedidos do quadro têm negócio de Vendas.
        </div>
      )}
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {lista.map(o=>(
          <Card key={o.vendasId}>
            <div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                <span style={{...F.title,fontSize:15,fontWeight:700,color:C.black}}>
                  {o.client||o.razaoSocial||"(cliente desconhecido)"}
                </span>
                <Tag label="Sem negócio de Vendas" color={C.red}/>
                {o.etapa&&<Tag label={o.etapa} color={C.gray600}/>}
              </div>
              <div style={{...F.body,fontSize:12,color:C.gray600,display:"flex",gap:16,flexWrap:"wrap"}}>
                <span>Vendas: <strong style={{color:C.gray700}}>{o.vendasId}</strong></span>
                {o.posvendaId&&<span>Pós-Venda: <strong style={{color:C.gray700}}>{o.posvendaId}</strong></span>}
                {o.bordadoId&&<span>Bordado: <strong style={{color:C.gray700}}>{o.bordadoId}</strong></span>}
                {o.cnpj&&<span>CNPJ: <strong style={{color:C.gray700}}>{o.cnpj}</strong></span>}
                {o.valor>0&&<span>Valor: <strong style={{color:C.gray700}}>{fmtR(o.valor)}</strong></span>}
                {o.dataVencimento&&<span>Prazo: <strong style={{color:C.gray700}}>{fmtVenc(o.dataVencimento,true)}</strong></span>}
              </div>
              {(o.itens||[]).length>0&&(
                <div style={{borderTop:`1px solid ${C.gray200}`,paddingTop:10}}>
                  <div style={{...F.body,fontSize:12,fontWeight:700,color:C.gray700,marginBottom:6}}>
                    {o.pecas} peça{o.pecas!==1?"s":""} em {o.qtdItens} item{o.qtdItens!==1?"ns":""}
                  </div>
                  {o.itens.map((it,ix)=>(
                    <div key={ix} style={{...F.body,fontSize:12,color:C.gray600,padding:"3px 0"}}>
                      {it.quantidade}× {it.nome} {it.tamanho&&`— ${it.tamanho}`}
                      {it.status&&<span style={{color:C.gray500}}> · {it.status}</span>}
                      {it.bordado&&it.direcionamento&&<span style={{color:C.purple}}> · bordado {it.direcionamento}</span>}
                    </div>
                  ))}
                </div>
              )}
...
// O SGP mudou para sgp.citerol.com.br. Esta faixa aparece SÓ no endereço antigo
// — a checagem é pelo hostname, então o mesmo código serve para os dois: quem
// já está no domínio novo não vê nada.
// Depois de 28/08 dá pra apagar este componente e a chamada dele.
const SGP_DOMINIO_NOVO = "sgp.citerol.com.br";
const SGP_URL_NOVA = "https://" + SGP_DOMINIO_NOVO;
const SGP_DESATIVA_EM = "28/08";
function ehEnderecoAntigo(){
  if(typeof window==="undefined")return false;
  const h=(window.location.hostname||"").toLowerCase();
  // Em desenvolvimento não mostra — só atrapalharia.
  if(h==="localhost"||h==="127.0.0.1"||h==="")return false;
  return h!==SGP_DOMINIO_NOVO;
}
function AvisoEnderecoAntigo(){
  if(!ehEnderecoAntigo())return null;
  return (
    <div style={{background:C.red,color:C.white,padding:"10px 16px",display:"flex",
      alignItems:"center",justifyContent:"center",gap:12,flexWrap:"wrap",flexShrink:0,
      ...F.body,fontSize:13.5,lineHeight:1.4,textAlign:"center"}}>
      <span style={{display:"inline-flex",alignItems:"center",gap:8}}>
        <Ic n="warn" s={17} c={C.white}/>
        <span>
          <strong>Este endereço será desativado em {SGP_DESATIVA_EM}.</strong>
          {" "}A partir de agora use o novo endereço do SGP:
        </span>
      </span>
      <a href={SGP_URL_NOVA}
         style={{background:C.white,color:C.red,borderRadius:6,padding:"6px 14px",
           textDecoration:"none",fontWeight:800,fontSize:13.5,whiteSpace:"nowrap"}}>
        {SGP_DOMINIO_NOVO} →
      </a>
      <span style={{fontSize:12,opacity:0.9}}>Atualize seu favorito.</span>
    </div>
  );
}
...
function Folha({emit,o,fmtDt}){
  ...
  return(
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      <div className="produto-row" style={{border:"1.5px solid #333",borderRadius:8,marginBottom:14,overflow:"hidden"}}>
        {/* Faixa do topo: nome do cliente + clifor + selos */}
        <div style={{background:"#e4e4e4",borderBottom:"1.5px solid #333",padding:"8px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
          <div style={{minWidth:0,flex:1,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            <span style={{...F.body,fontSize:9.5,color:"#000",fontWeight:800,textTransform:"uppercase",letterSpacing:"0.08em"}}>Cliente</span>
            <span style={{...F.body,fontSize:14.5,fontWeight:800,color:"#000",lineHeight:1.2}}>{o.cliente||"—"}</span>
            {o.clifor&&<span style={{...F.body,fontSize:11,fontWeight:700,color:"#111",background:"#fff",border:"1.5px solid #333",borderRadius:5,padding:"1px 7px",whiteSpace:"nowrap"}}>Clifor {o.clifor}</span>}
          </div>
          {(() => {
        ...
        {(() => {
          ...
          // Natureza da operação: define o tratamento fiscal do pedido
          // (venda, remessa, bonificação...). Quem confere precisa ver.
          if (o.natureza) linhas.push([["Natureza da operação", o.natureza]]);
          const com = [];
          ...
          // [FRETE] Em quantas e quais caixas o TMS previu consolidar o pedido.
          // Vai na folha porque quem embala trabalha com o papel na mão — se
          // isso só existisse na tela, o conferente teria que ir olhar no SGP
          // no meio da embalagem.
          // ⚠ Sempre imprime a linha, mesmo sem cotação. Linha ausente é
          // ambígua: quem embala não sabe se o pedido não foi cotado ou se a
          // folha saiu incompleta. Dizendo "sem cotação" a dúvida acaba.
          const fvImp = o.freteVolumes || null;
          const qtd = fvImp && fvImp.temPrevisto ? fvImp.previsto.qtd : 0;
          linhas.push([["Volumes previstos",
            fvImp && fvImp.temPrevisto
              ? (fvImp.previsto.texto || "—") + (qtd ? "  ·  " + qtd + " volume" + (qtd !== 1 ? "s" : "") : "")
              : "Pedido sem cotação de volumes"]]);
          // Se a expedição já registrou, mostra também — reimpressão depois de
          // embalar serve de conferência.
          if (fvImp && fvImp.temRealizado) {
            const qtdR = fvImp.realizado.qtd;
            linhas.push([["Volumes realizados",
              (fvImp.realizado.texto || "—") + (qtdR ? "  ·  " + qtdR + " volume" + (qtdR !== 1 ? "s" : "") : "")
              + (fvImp.divergente ? "   (DIFERENTE DO PREVISTO)" : "")]]);
          }
          if (!linhas.length) return null;
          const tdRot = {...F.body,fontSize:9.5,fontWeight:800,color:"#000",textTransform:"uppercase",letterSpacing:"0.06em",
            padding:"5px 10px",whiteSpace:"nowrap",verticalAlign:"middle",background:"#ebebeb",width:1};
          ...
          return (
            <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"auto"}}>
              <tbody>
                {linhas.map((par,i)=>(
                  <tr key={i} style={{borderTop:i===0?"none":"1.5px solid #333"}}>
                    {par.map(([rot,val],j)=>(
                      <Fragment key={j}>
                        <td style={{...tdRot,borderLeft:j>0?"1.5px solid #333":"none"}}>{rot}:</td>
...
function AguardandoOutroPedido({user}){
  ...
  // Busca pelo nº do Linx (como a operação chama), pelo ID do HubSpot, pelo
  // cliente e também pelo pedido AGUARDADO — a pergunta comum é "quem está
  // esperando o 219358?".
  const q=busca.trim().toLowerCase();
  const lista=items.filter(o=>{
    if(!q)return true;
    return [o.pedidoLinx,o.vendasId,o.client,o.pedidoDependencia,o.dependenciaLinx,o.dependenciaCliente,o.motivo]
      .map(x=>String(x||"").toLowerCase()).join(" ").includes(q);
  });

  return(
    <div style={{padding:24}}>
      <PageH title="Aguardando Outro Pedido" sub="Pedidos que dependem de outro para prosseguir (faturamento conjunto, material atrelado, etc)." onRefresh={carregar} refreshing={loading}/>
      <div style={{marginBottom:14,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        <input value={busca} onChange={e=>setBusca(e.target.value)}
          placeholder="Buscar por pedido, cliente ou pedido aguardado..."
          style={{flex:1,minWidth:260,maxWidth:460,...F.body,fontSize:13,padding:"10px 12px",border:`1px solid ${C.gray200}`,borderRadius:8,outline:"none",boxSizing:"border-box"}}/>
        {!loading&&<span style={{...F.body,fontSize:12,color:C.gray500}}>{lista.length} de {items.length} pedido{items.length!==1?"s":""}</span>}
      </div>
      ...
      {loading?<div style={{padding:40,textAlign:"center",...F.body,color:C.gray400}}>Carregando...</div>
       :lista.length===0?<Card><div style={{padding:40,textAlign:"center",...F.body,color:C.gray500}}>{q?"Nenhum pedido encontrado para a busca.":"Nenhum pedido aguardando outro no momento."}</div></Card>
       :<div style={{display:"flex",flexDirection:"column",gap:10}}>
         {lista.map(o=>(
           <div key={o.id} style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,padding:14,borderLeft:`4px solid ${C.blue}`}}>
             <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,marginBottom:8,flexWrap:"wrap"}}>
               <div style={{minWidth:0,flex:1}}>
                 <div style={{...F.body,fontSize:14,fontWeight:700,color:C.black}}>
                   {o.pedidoLinx?<>PED - {o.pedidoLinx} <span style={{fontWeight:400,color:C.gray500}}>| {o.vendasId}</span></>:<>PED - {o.vendasId}</>}
                 </div>
             ...
             <div style={{background:C.blue+"12",borderLeft:`3px solid ${C.blue}`,borderRadius:4,padding:"8px 10px",marginBottom:8,...F.body,fontSize:12,color:"#1e40af",fontWeight:600,lineHeight:1.4}}>
               <div><strong>🔗 Aguardando pedido:</strong>{" "}
                 {o.dependenciaLinx
                   ? <>PED - {o.dependenciaLinx} <span style={{fontWeight:400}}>| {o.pedidoDependencia}</span>{o.dependenciaCliente?` · ${o.dependenciaCliente}`:""}</>
                   : (o.pedidoDependencia||"—")}
               </div>
...
function AppInner(){
  ...
  const contagensMenu=useMemo(()=>{
    ...
    // [SILK/DTF] Não é etapa — é personalização da peça. Como não existe
    // porEtapa["Silk/DTF"], o item do menu ficava sem contador enquanto todos
    // os outros mostravam. Conta varrendo o quadro: pedidos com peça de
    // silk/DTF que a produção ainda não marcou como aplicada.
    try{
      const vistos=new Set();
      for(const nome of Object.keys(porEtapa)){
        for(const c of (porEtapa[nome]?.items||[])){
          if(c.temSilkDtf!==true||c.silkDtfConcluido===true)continue;
          const k=String(c.vendasId||c.id);
          if(vistos.has(k))continue;
          vistos.add(k);
        }
      }
      if(vistos.size>0)out.silk_dtf=vistos.size;
    }catch{}
    return out;
  },[snapMenu.data]);
  ...
  // A faixa vai ANTES do login também: quem chega pelo link velho precisa ver o
  // aviso mesmo sem entrar no sistema.
  if(!user)return <div style={{display:"flex",flexDirection:"column",height:"100dvh"}}>
    <AvisoEnderecoAntigo/>
    <div style={{flex:1,overflow:"auto"}}><Login onLogin={doLogin}/></div>
  </div>;
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div className="sgp-scroll" style={{flex:1,overflowY:"auto",paddingBottom:isMobile?70:0}}>
            ...
            {page==="relatorio_pedidos"&&<RelatorioPedidos onOpen={setSel}/>}
            ...
            {page==="pedidos_sem_venda"&&<PedidosSemVenda/>}
          </div>
        </div>
      </div>
