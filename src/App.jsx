...
function snapRemoverPedido(vendasId, etapa){
  const d = _snapState.data;
  if (!d || !d.porEtapa || !vendasId) return;
  const alvo = String(vendasId);
  let mexeu = false;
  for (const nome of Object.keys(d.porEtapa)) {
    if (etapa && nome !== etapa) continue;
    const g = d.porEtapa[nome];
    if (!g || !Array.isArray(g.items)) continue;
    const antes = g.items.length;
    g.items = g.items.filter(it => String(it.vendasId || "") !== alvo);
    if (g.items.length !== antes) mexeu = true;
  }
  if (mexeu) {
