...
function AcaoTab({order,me,uploadFile,setUploadFile,uploadName,setUploadName,obsText,setObsText,actionDone,setActionDone,actionMsg,setActionMsg,itemSel,itemDest,nSel,allDestDefined,skus,itensDirecionaveis,toggleItemSel,selAllItems,setDestSel,setDestAll,setDestOne,onAction,isMobile,loadingDet,setTemPendencias}){
  ...
  const moveConfig={
    "Em Separação":             {title:"ENVIAR PARA ANÁLISE PCP", sub:"Use quando o pedido NÃO tem itens para separar e precisa ir direto para a Análise PCP. Se houver itens, faça a separação normal.", btn:"Enviar para Análise PCP", icon:"arrow", color:C.red, next:"Análise PCP"},
  ...
  // Em Separação: o envio direto pra PCP só vale quando NÃO há itens separáveis.
  const semItensSep = (order.items&&order.items.length>0)
    ? order.items.filter(i=>!i.naoSeparavel).length===0
    : Number(order.totalItensSeparacao||0)===0;
  const bloqueiaSep = etapa==="Em Separação" && !semItensSep;
  ...
  return(
    <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
      ...
      {bloqueiaSep&&<div style={{...F.body,fontSize:12,color:"#92400e",background:"#fffbeb",border:`1px solid ${C.amber}`,borderRadius:6,padding:"8px 12px"}}>Este pedido tem itens para separar — faça a separação normal. O envio direto para Análise PCP é só para pedidos <strong>sem itens</strong>.</div>}
      <button disabled={(etapa==="Expedição"&&!bipReady)||bloqueiaSep}
        onClick={async()=>{
          try{
            if(etapa==="Expedição"){
        ...
        style={{background:((etapa==="Expedição"&&!bipReady)||bloqueiaSep)?C.gray300:moveConfig.color,color:C.white,border:"none",borderRadius:8,padding:"12px 28px",cursor:((etapa==="Expedição"&&!bipReady)||bloqueiaSep)?"not-allowed":"pointer",...F.body,fontWeight:700,fontSize:14,display:"flex",alignItems:"center",gap:8,alignSelf:"flex-start"}}>
        ...
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.gray200}`,display:"flex",flexDirection:"column",gap:12,flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            ...
            {me&&me.admin&&!order.bordadoId&&(order.bordadosJson||[]).some(b=>b&&(b.sku||b.productId))&&<button
              ...
              onClick={async()=>{
                if(!confirm("Este pedido tem arquivos de bordado mas não entrou no fluxo de bordado (não está em Programação). Gerar o bordado na Programação agora? Os itens com bordado também serão marcados."))return;
                setGerandoBordado(true);
                try{
                  const r=await apiFetch(`/gerar-bordado/${order.vendasId}`,"POST",{ctx:{executor:me?.nome||me?.email||"Usuário SGP"}});
                  if(r.success){ alert(`Bordado gerado com ${r.programacoes} programação(ões)${typeof r.itensMarcados==="number"?` e ${r.itensMarcados} item(ns) marcado(s) como bordado`:""}. O pedido vai aparecer na Programação em instantes.`); onClose(); }
                  else alert("Erro: "+(r.error||"desconhecido"));
                }catch(e){alert("Erro: "+e.message);}
                finally{setGerandoBordado(false);}
              }}
              title="Gerar o bordado na Programação a partir dos arquivos já anexados"
        ...
        <div className="sgp-scroll" style={{flex:1,overflow:"auto",minHeight:0}}>
          ...
          {tab==="info"&&<div style={{padding:20,display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:10}}>
              {[["Cliente",order.client],["CNPJ",order.cnpj],["Razão Social",order.razaoSocial],["Vendedor",order.vendedor],["Telefone",order.tel],["E-mail",order.email],["Valor",fmtR(order.valor)],["Condição de Pgto",order.condicaoPagamento],["Prazo Faturamento do Pedido", _loadingDet?"⏳ Carregando...":(dataVencimento(order)?fmtD(dataVencimento(order)):"A definir (sem amostra aprovada)")],["Emissão",fmtD(order.dataFechamento||order.entradaAt)]].map(([k,v])=>(
                <div key={k} style={{background:C.gray50,borderRadius:6,padding:"10px 12px",border:`1px solid ${C.gray200}`}}>
                  <div style={{...F.body,fontSize:10,color:C.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:4}}>{k}</div>
...
function Folha({emit,o,fmtDt}){
  ...
  return(
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      {/* Cliente */}
      <div className="produto-row" style={{background:"#f0f0f0",border:"1px solid #c0c0c0",borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        <div style={{minWidth:0,flex:1}}>
