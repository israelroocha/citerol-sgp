...
function Folha({emit,o,fmtDt}){
  ...
  const STATUS_CFG={
    completa: {lbl:"PEDIDO COMPLETO",         sub:"TODAS AS PEÇAS SEPARADAS",                                                    cor:"#0d4d24",bg:"#e8f5ec",bd:"#4a8f5f", letra:"V"},
    parcial:  {lbl:"PEDIDO SEPARADO PARCIAL", sub:totalPecas>0?`${totalSeparado} DE ${totalPecas} PEÇAS SEPARADAS`:"ATENÇÃO: HÁ PEÇAS PENDENTES", cor:"#7a2a06",bg:"#fce8d5",bd:"#c46a2f", letra:"P"},
    pendente: {lbl:"SEPARAÇÃO PENDENTE",      sub:"NENHUMA PEÇA FOI SEPARADA AINDA",                                             cor:"#5c0f0f",bg:"#f5d5d5",bd:"#9e3d3d", letra:"X"},
  };
  ...
  // Caixinha padronizada com letra dentro — legível em preto e branco
  const Box=({letra,cor,tam=14})=>(
    <span style={{
      display:"inline-flex",alignItems:"center",justifyContent:"center",
      width:tam,height:tam,border:`2px solid ${cor}`,borderRadius:2,
      fontSize:tam-4,fontWeight:900,color:cor,lineHeight:1,
      fontFamily:"'Oswald',monospace",flexShrink:0,
    }}>{letra}</span>
  );
  const pilProduto=(p)=>{
    if(p.statusProduto==="completo")return {lbl:"SEPARADO",cor:"#0d4d24",bg:"#e8f5ec",bd:"#4a8f5f", letra:"V"};
    if(p.statusProduto==="parcial") return {lbl:`PARCIAL ${p.gradesSeparadas||0}/${p.qtdGrades||0}`,cor:"#7a2a06",bg:"#fce8d5",bd:"#c46a2f", letra:"P"};
    return {lbl:"PENDENTE",cor:"#5c0f0f",bg:"#f5d5d5",bd:"#9e3d3d", letra:"X"};
  };
  return(
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      <div className="produto-row" style={{display:"flex",alignItems:"center",gap:16,borderBottom:`2px solid #555`,paddingBottom:14,marginBottom:14}}>
        ...
        <div style={{textAlign:"right",flexShrink:0}}>
          <div style={{...F.title,fontSize:10,fontWeight:700,color:"#333",letterSpacing:"0.1em"}}>FOLHA DE SEPARAÇÃO</div>
      ...
      {/* Banner de status — caixinha com letra pra distinguir sem cor */}
      <div className="produto-row" style={{marginBottom:14,borderRadius:10,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,border:`2px solid ${st.bd}`,background:st.bg,color:st.cor}}>
        <div style={{display:"flex",alignItems:"center",flex:1,gap:12}}>
          <Box letra={st.letra} cor={st.cor} tam={26}/>
          <div>
      ...
      <div style={{display:"flex",gap:14,fontSize:10,fontWeight:700,color:"#333",marginBottom:10,flexWrap:"wrap",alignItems:"center"}}>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><Box letra="V" cor="#0d4d24" tam={12}/> SEPARADO</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><Box letra="X" cor="#5c0f0f" tam={12}/> PENDENTE</span>
        <span style={{display:"inline-flex",alignItems:"center",gap:5}}><Box letra="P" cor="#7a2a06" tam={12}/> PARCIAL</span>
      </div>
        ...
        {(o.produtos||[]).map((p,i)=>{
          ...
          <div key={i} className="produto-row" style={{display:"flex",gap:12,alignItems:"stretch",border:"1px solid #555",borderRadius:8,padding:8}}>
            ...
            <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                ...
                <span style={{...F.title,fontSize:9.5,fontWeight:800,letterSpacing:"0.05em",padding:"2px 7px",borderRadius:4,background:pil.bg,color:pil.cor,border:`1.5px solid ${pil.bd}`,display:"inline-flex",alignItems:"center",gap:5}}>
                  <Box letra={pil.letra} cor={pil.cor} tam={12}/> {pil.lbl}
                </span>
              </div>
              <div style={{...F.title,fontSize:14,fontWeight:800,color:"#000",lineHeight:1.15}}>{p.nome}</div>
              {/* Grades com caixinha V/X pra distinguir em B&W */}
              <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:3}}>
                {(p.grades||[]).map((g,j)=>{
                  const sep=g.separado;
                  const gCor=sep?"#0d4d24":"#5c0f0f";
                  return (
                    <span key={j} style={{
                      ...F.body,fontSize:11,fontWeight:700,
                      background:sep?"#e8f5ec":"#fff",
                      color:sep?"#0d4d24":"#000",
                      border:sep?"1.5px solid #4a8f5f":"2px dashed #5c0f0f",
                      borderRadius:4,padding:"2px 8px",
                      display:"inline-flex",alignItems:"center",gap:5,
                      textDecoration:sep?"line-through":"none",
                    }}>
                      <span style={{textDecoration:"none",display:"inline-flex"}}>
                        <Box letra={sep?"V":"X"} cor={gCor} tam={12}/>
                      </span>
            ...
            <div style={{textAlign:"center",flexShrink:0,paddingLeft:10,borderLeft:"1px solid #c0c0c0",display:"flex",flexDirection:"column",justifyContent:"center",minWidth:78}}>
              ...
              {p.totalSeparado>0&&p.totalSeparado<p.total&&<div style={{...F.body,fontSize:9,color:"#7a2a06",fontWeight:800,marginTop:4}}>{p.totalSeparado} SEP.</div>}
            </div>
          </div>
          );
        })}
      ...
      <div className="produto-row" style={{marginTop:14,paddingTop:10,borderTop:"2px solid #555",display:"flex",justifyContent:"space-between",...F.body,fontSize:13,fontWeight:800}}>
        <span>{(o.produtos||[]).length} produto(s){(o.bordados||[]).length?` · ${o.bordados.length} bordado(s)`:""}</span>
        <span style={{color:"#7a0007"}}>Total de peças: {totalPecas}</span>
      </div>
    </div>
  );
}
