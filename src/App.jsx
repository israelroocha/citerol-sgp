...
const ordenarPorPrioridade=(arr)=>[...arr].sort((a,b)=>{
  ...
  // 1) Ambos com vencimento: mais cedo (mais vencido) primeiro.
  if(da&&db){ const d=new Date(da)-new Date(db); if(d)return d; }
  // 2) Só um tem vencimento: quem tem prazo definido vem antes.
  else if(da&&!db)return -1;
  else if(!da&&db)return 1;
  // 3) Sem vencimento (ex.: Programação — antes da amostra) OU empate:
  //    desempata pela EMISSÃO mais antiga primeiro (FIFO). Assim os pedidos
  //    que estão esperando há mais tempo sobem pro topo, em vez de ficar na
  //    ordem crua do snapshot (mais recentes em cima).
  const ea=a.dataFechamento||a.entradaAt||null, eb=b.dataFechamento||b.entradaAt||null;
  if(!ea&&!eb)return 0;
  if(!ea)return 1;
  if(!eb)return -1;
  return new Date(ea)-new Date(eb); // mais antiga primeiro
});
