    ...
    try {
      return await fetch(`${WORKER_URL}${path}`, {
        ...
        headers: {
          ...
          // Sessão individual do usuário. As rotas do bordador externo exigem
          // este cabeçalho — a chave do app sozinha não identifica quem é.
          ...(function(){ try{ const t=localStorage.getItem("sgp_sessao"); return t?{"X-Sessao":t}:{}; }catch{ return {}; } })(),
        },
  ...
  return(
    <div style={{padding:20}}>
      ...
      {aba==="finalizadas"&&<>
        ...
        {listaFin.map(o=>(
          <Card key={o.id} style={{marginBottom:12,borderLeft:`3px solid ${C.green}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
    ...
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      <div className="produto-row" style={{background:"#f0f0f0",border:"1px solid #c0c0c0",borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        <div style={{minWidth:0,flex:1}}>
          ...
          {/* Razão social: só aparece quando difere do nome fantasia, pra não
              repetir a mesma informação duas vezes na folha. */}
          {(o.razaoSocial&&o.razaoSocial.trim()&&o.razaoSocial.trim().toLowerCase()!==String(o.cliente||"").trim().toLowerCase())&&
            <div style={{...F.body,fontSize:11.5,color:"#222",marginTop:2,fontWeight:600}}>Razão social: {o.razaoSocial}</div>}
