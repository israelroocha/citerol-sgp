...
function Folha({emit,o,fmtDt}){
  ...
  const STATUS_CFG={
    completa: {lbl:"PEDIDO COMPLETO",         sub:"TODAS AS PEÇAS SEPARADAS",                                                    cor:"#0d4d24",bg:"#e8f5ec",bd:"#4a8f5f", icone:"✓"},
    parcial:  {lbl:"PEDIDO SEPARADO PARCIAL", sub:totalPecas>0?`${totalSeparado} DE ${totalPecas} PEÇAS SEPARADAS`:"ATENÇÃO: HÁ PEÇAS PENDENTES", cor:"#7a2a06",bg:"#fce8d5",bd:"#c46a2f", icone:"⟡"},
    pendente: {lbl:"SEPARAÇÃO PENDENTE",      sub:"NENHUMA PEÇA FOI SEPARADA AINDA",                                             cor:"#5c0f0f",bg:"#f5d5d5",bd:"#9e3d3d", icone:"○"},
  };
  ...
  const pilProduto=(p)=>{
    if(p.statusProduto==="completo")return {lbl:"SEPARADO",cor:"#0d4d24",bg:"#e8f5ec",bd:"#4a8f5f", icone:"✓"};
    if(p.statusProduto==="parcial") return {lbl:`PARCIAL ${p.gradesSeparadas||0}/${p.qtdGrades||0}`,cor:"#7a2a06",bg:"#fce8d5",bd:"#c46a2f", icone:"⟡"};
    return {lbl:"PENDENTE",cor:"#5c0f0f",bg:"#f5d5d5",bd:"#9e3d3d", icone:"○"};
  };
  return(
    <div className="folha-print" style={{background:C.white,border:`1px solid ${C.gray200}`,borderRadius:8,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",width:"100%",maxWidth:820,padding:32,boxSizing:"border-box",color:"#000"}}>
      ...
      {/* Banner de status — com ícone gigante pra distinguir sem cor */}
      <div className="produto-row" style={{marginBottom:14,borderRadius:10,padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,border:`2px solid ${st.bd}`,background:st.bg,color:st.cor}}>
        <div style={{display:"flex",alignItems:"center",flex:1}}>
          <span style={{fontSize:22,marginRight:12,fontWeight:900}}>{st.icone}</span>
          <div>
      ...
      <div style={{display:"flex",gap:14,fontSize:10,fontWeight:700,color:"#333",marginBottom:10,flexWrap:"wrap"}}>
        <span>☑ SEPARADO</span>
        <span>☐ PENDENTE</span>
        <span>✓ PRODUTO COMPLETO</span>
        <span>⟡ PARCIAL</span>
        <span>○ NADA SEPARADO</span>
      </div>
          ...
          <div key={i} className="produto-row" style={{display:"flex",gap:12,alignItems:"stretch",border:"1px solid #555",borderRadius:8,padding:8}}>
            ...
            <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",justifyContent:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                ...
                <span style={{...F.title,fontSize:9.5,fontWeight:800,letterSpacing:"0.05em",padding:"2px 7px",borderRadius:4,background:pil.bg,color:pil.cor,border:`1.5px solid ${pil.bd}`,display:"inline-flex",alignItems:"center",gap:3}}>
                  <span style={{fontSize:11}}>{pil.icone}</span> {pil.lbl}
                </span>
              </div>
              <div style={{...F.title,fontSize:14,fontWeight:800,color:"#000",lineHeight:1.15}}>{p.nome}</div>
              {/* Grades com ☑/☐ pra distinguir em B&W */}
                ...
                {(p.grades||[]).map((g,j)=>{
                  ...
                  return (
                    <span key={j} style={{
                      ...
                      <span style={{fontWeight:900,fontSize:13,textDecoration:"none",color:sep?"#0d4d24":"#5c0f0f"}}>{sep?"☑":"☐"}</span>
